...
const orders = pgTable('orders', { id: integer('id').primaryKey(), shippedDate: text('shipped_date'), shipName: text('ship_name'), shipCity: text('ship_city'), shipCountry: text('ship_country') });
...
const Q: Record<string, { desc: string; drizzle: Impl; kysely: Impl; zmdb: Impl }> = {
  'p1 customers-list': {
    ...
    kysely: async () => void (await k.selectFrom('customers').selectAll().orderBy('id').limit(50).offset(100).execute()),
    zmdb: async () => { const c = qc.selectFrom('customers').orderBy('id', 'asc').limit(50).offset(100).compile(); await z(c.text, c.parameters as unknown[]); },
  },
  'p2 customer-by-id': {
    desc: 'customer by id',
    ...
    zmdb: async () => { const c = qc.selectFrom('customers').where('id', '=', 42).compile(); await z(c.text, c.parameters as unknown[]); },
  },
  'p3 customer-search (FTS)': {
    desc: 'full-text search on company_name',
    drizzle: async () => void (await ddb.select().from(customers).where(sql`to_tsvector('english', ${customers.companyName}) @@ to_tsquery('english', ${'ltd'})`)),
    kysely: async () => void (await k.selectFrom('customers').selectAll().where(ksql<boolean>`to_tsvector('english', company_name) @@ to_tsquery('english', ${'ltd'})`).execute()),
  ...
  'p4 employees-list': {
    ...
    zmdb: async () => { const c = qc.selectFrom('employees').orderBy('id', 'asc').limit(50).offset(0).compile(); await z(c.text, c.parameters as unknown[]); },
  },
  'p5 employee+recipient': {
    ...
    kysely: async () => void (await k.selectFrom('employees').leftJoin('employees as r', 'r.id', 'employees.recipient_id').selectAll('employees').where('employees.id', '=', 5).execute()),
  ...
  'p6 suppliers-list': {
    ...
    zmdb: async () => { const c = qc.selectFrom('suppliers').orderBy('id', 'asc').limit(50).offset(0).compile(); await z(c.text, c.parameters as unknown[]); },
  },
  'p7 supplier-by-id': {
    desc: 'supplier by id',
    ...
    zmdb: async () => { const c = qc.selectFrom('suppliers').where('id', '=', 3).compile(); await z(c.text, c.parameters as unknown[]); },
  },
  'p8 products-list': {
    ...
    zmdb: async () => { const c = qc.selectFrom('products').orderBy('id', 'asc').limit(50).offset(0).compile(); await z(c.text, c.parameters as unknown[]); },
  },
  'p9 product+supplier': {
    desc: 'product by id WITH supplier join',
    drizzle: async () => void (await ddb.select().from(products).leftJoin(suppliers, eq(suppliers.id, products.supplierId)).where(eq(products.id, 7))),
    kysely: async () => void (await k.selectFrom('products').leftJoin('suppliers', 'suppliers.id', 'products.supplier_id').selectAll().where('products.id', '=', 7).execute()),
    zmdb: DNF, // no join builder
  },
  'p10 product-search (FTS)': {
    desc: 'full-text search on product name',
    drizzle: async () => void (await ddb.select().from(products).where(sql`to_tsvector('english', ${products.name}) @@ to_tsquery('english', ${'chai'})`)),
    kysely: async () => void (await k.selectFrom('products').selectAll().where(ksql<boolean>`to_tsvector('english', name) @@ to_tsquery('english', ${'chai'})`).execute()),
    zmdb: DNF, // no FTS builder
  },
  'p11 orders+agg list': {
    desc: 'orders w/ aggregated details (GROUP BY, computed cols), paginated',
    drizzle: async () => void (await ddb.select({ id: orders.id, productsCount: sql`count(${details.productId})::int`, quantitySum: sql`sum(${details.quantity})::int` }).from(orders).leftJoin(details, eq(details.orderId, orders.id)).groupBy(orders.id).orderBy(asc(orders.id)).limit(50).offset(0)),
    kysely: async () => void (await k.selectFrom('orders').leftJoin('order_details', 'order_details.order_id', 'orders.id').select(['orders.id']).select(ksql`count(order_details.product_id)::int`.as('products_count')).groupBy('orders.id').orderBy('orders.id').limit(50).offset(0).execute()),
    zmdb: DNF, // no join / GROUP BY / aggregate builder
  },
  'p12 order+agg by-id': {
    desc: 'single order w/ aggregated details',
    drizzle: async () => void (await ddb.select({ id: orders.id, productsCount: sql`count(${details.productId})::int` }).from(orders).leftJoin(details, eq(details.orderId, orders.id)).where(eq(orders.id, 10500)).groupBy(orders.id)),
    kysely: async () => void (await k.selectFrom('orders').leftJoin('order_details', 'order_details.order_id', 'orders.id').select(['orders.id']).select(ksql`count(order_details.product_id)::int`.as('products_count')).where('orders.id', '=', 10500).groupBy('orders.id').execute()),
    zmdb: DNF, // no join / aggregate builder
  },
  'p13 order-with-details': {
    desc: 'order + its line items',
    drizzle: async () => void (await ddb.select().from(orders).leftJoin(details, eq(details.orderId, orders.id)).where(eq(orders.id, 10500))),
    kysely: async () => void (await k.selectFrom('orders').leftJoin('order_details', 'order_details.order_id', 'orders.id').selectAll().where('orders.id', '=', 10500).execute()),
    // zmdb pattern: explicit two-query populate (parent + batched children) — expressible with the CRUD builder.
    zmdb: async () => { const o = qc.selectFrom('orders').where('id', '=', 10500).compile(); await z(o.text, o.parameters as unknown[]); const d = qc.selectFrom('order_details').where('order_id', '=', 10500).compile(); await z(d.text, d.parameters as unknown[]); },
  },
};
...
for (const [name, q] of Object.entries(Q)) {
  ...
  for (const orm of ORMS) {
    const impl = q[orm];
    if (impl === DNF) { dnfCount[orm]++; present[orm] = false; continue; }
    present[orm] = true;
