...
The `User` shape is declared once, as an interface, in `model.ts`. `UserCreate` is
`CreateDTO<User>` — the `id` column carries `Serial`, so it is absent from the
insert shape without anybody saying so — and the check for it is compiled by
`zmdb-codegen` into `model.zmdb.generated.js`, which is committed beside the
interface. `app.ts` imports the compiled function and hands it to `validateWith`.
There is no schema value in the process and nothing is resolved at boot.

Two earlier arrangements are recorded in `app.ts`'s comments because each was a
weaker claim than it looked. The first built the descriptor inside `app.ts` from a
`columnKind` switch, i.e. measured a validator no user of the library gets. The
second replaced that with `objectTypeFromIR(irFromSchema(UserSchema), 'create')`
at boot — the real descriptor, but still walked per request.

The declaration lives in `model.ts` rather than in `app.ts` because `app.ts`
imports `../../../packages/web/dist`, gitignored build output that no compiler can
resolve in a fresh checkout, and the codegen needs a compiler. `tsconfig.json`
here lists `model.ts` and its generated witness, and `scripts/typecheck.mjs`
checks that project like any other.
...
- **The published numbers do not measure validation.** Upstream's `POST /user`
  sends no body, and `run.sh` drives `oha` without `-d` to match it, so the
  route's `validateBody` hook sees `undefined` and returns immediately — the
  compiled check is registered and reachable but never called during a load run.
  That is why swapping the boot-resolved descriptor for the compiled check left
  `framework-results.json` untouched rather than obsolete. A validation number is
