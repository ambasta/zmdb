...
const {
  ...
  const modelText = `${BANNER('harness/validation/model.ts')}
...
`;
  ...
  const aotText = `${BANNER('harness/validation/aot-source.ts')}${strip(result.code, [
  ...
  const shallowText = `${BANNER('harness/validation/shallow-source.ts')}${strip(shallowResult.code, [
    /^import \{[^}]*\} from '[^']*utilities\/index\.[jt]s';\n/m,
  ])}`;

  return { modelText, aotText, shallowText };
});
