...
const { model, aot, shallow } = withSession({ project: PROJECT }, session => {
  ...
  const modelCode = `${BANNER('harness/validation/model.ts')}
...
`;
  ...
  const aotCode = `${BANNER('harness/validation/aot-source.ts')}${strip(result.code, [
    /^import \{[^}]*\} from '@zmdb\/validator';\n/m,
    /^import type \{[^}]*\} from '[^']*';\n/m,
  ])}`;
  ...
  const shallowCode = `${BANNER('harness/validation/shallow-source.ts')}${strip(shallowResult.code, [
    /^import \{[^}]*\} from '@zmdb\/validator';\n/m,
  ])}`;

  return { model: modelCode, aot: aotCode, shallow: shallowCode };
});
