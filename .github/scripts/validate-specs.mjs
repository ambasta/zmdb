...
try {
  ...
  let diffOutput = '';
  try {
    diffOutput = execSync('git diff --name-only origin/main...HEAD', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] });
  } catch {
    try {
      diffOutput = execSync('git diff --name-only HEAD~1 HEAD', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] });
    } catch {
  ...
  for (const specPath of specFiles) {
    ...
    if (pkgHasChanges) {
      ...
      if (uncheckedInSpec.length > 0) {
        errors.push(`Package '@zmdb/${pkg}' has source changes but ${relPath} contains ${uncheckedInSpec.length} unverified checklist item(s)`);
      }
    }
  }
}
