...
const MAP = {
  ...
  72: {
    impl: 'benchmarks/src/report.ts',
    tests: 'report.spec.ts (6) + generate-results.spec.ts',
    note: 'toMarkdown/toJson emit deterministically-ordered reports with explicit DNF rows; assertNoSilentSkips enforces the honesty policy (primary target must cover every in-scope case; all results schema-valid) — throws ReportError otherwise. RESULTS.md generation refactored onto this module.',
  },
};
