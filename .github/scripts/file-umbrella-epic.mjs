...
const EPIC = {
  ...
  subs: [
    ...
    { key: 'E1', kind: 'spec', title: 'zmdb top-level API surface (what re-exports at root vs subpath, collision policy)', blockedByX: ['A1', 'B1'], blockedBy: [] },
    { key: 'E2', kind: 'tests', title: 'zmdb re-export surface (public API + type-level re-export assertions)', blockedByX: [], blockedBy: ['E1'] },
    { key: 'E3', kind: 'impl', title: 'zmdb package: re-exports + build + publish wiring', blockedByX: ['A3', 'A6', 'B3', 'B4'], blockedBy: ['E2'] },
    { key: 'E4', kind: 'docs', title: 'Make `zmdb` the default install in Quick Start / install docs', pages: ['installation', 'quick-start', 'introduction'], blockedByX: ['D3'], blockedBy: ['E3'] },
  ],
};
...
let existing = [];
if (RUN) existing = JSON.parse(gh(['issue', 'list', '--repo', REPO, '--state', 'all', '--limit', '700', '--json', 'number,title']));
...
function subBody(epicNum, s, blockersText) {
  return [
    ...
    s.kind === 'spec' ? `Freeze the spec for: **${s.title}** (SPEC & TDD first — no implementation).`
      : s.kind === 'tests' ? `Author FAILING tests for: **${s.title}** (red), incl. type-level re-export assertions.`
...
for (const s of EPIC.subs) keyToNum[s.key] = create({ title: subTitle(s), body: subBody(epicNum, s, ''), labels: subLabels(s) });
...
if (RUN) {
  ...
  const list = EPIC.subs.map((s) => {
    const bs = [...(s.blockedBy || []).map((k) => '#' + keyToNum[k]), ...(s.blockedByX || []).map((k) => '#' + X[k])];
    return `- [ ] #${keyToNum[s.key]}${bs.length ? ` (blocked by ${bs.join(', ')})` : ''}`;
  }).join('\n');
  gh(['issue', 'edit', String(epicNum), '--repo', REPO, '--body-file', '-'], epicBody0.replace('_(populated below)_', list));
