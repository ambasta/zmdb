...
function graphql(q, v) {
  const a = ['api', 'graphql', '-f', `query=${q}`];
  for (const [k, val] of Object.entries(v ?? {})) a.push('-F', `${k}=${val}`);
...
const EPICS = [
  {
    ...
    subs: [
      {
        t: 'Freeze spec: plugin packaging + <T>→inlined-JS contract + benchmark target',
        depends: 'none',
        spec: [
          'Define the transformer plugin surface (ts-patch/unplugin) and how is<T>/assert<T>/validate<T> are recognized.',
          'Freeze the emitted-JS contract: monomorphic, allocation-free, early-exit; no TypeDescriptor walk.',
          'Freeze the benchmark acceptance target: AOT ≥5× runtime and competitive with typia/TypeBox on the moltar suite.',
        ],
        tests: [
          'Failing golden tests: given a source using is<T>()/assert<T>(), expect specific inlined JS (string compare).',
        ],
        accept: ['Committed SPEC.md with before/after fixtures + the acceptance target.', 'Tests compile and FAIL.'],
      },
      {
        t: 'Type-driven codegen from a real TS type T (not a runtime descriptor)',
        depends: 'the spec-freeze sub-issue',
        spec: [
          'Read the checked type from the TS checker; emit straight-line per-field checks with early return false.',
          'Handle nested objects, arrays, unions, optional/nullable.',
        ],
      ...
      {
        t: 'Package as ts-patch / unplugin build plugin + fixture build',
        depends: 'type-driven codegen',
        spec: [
          'Ship a plugin consumable via ts-patch and unplugin; a fixture project builds with it and emits inlined output.',
        ],
        tests: [
          'Integration test: build the fixture; assert is<T>()/assert<T>() calls are replaced (no descriptor walk in output).',
        ],
        accept: ['Fixture build emits inlined validators; integration test green.'],
      },
      {
        t: 'Re-run moltar suite with the AOT build + record real numbers',
        depends: 'the plugin package',
      ...
      {
        t: 'Acceptance gate: AOT ≥5× runtime & competitive with typia/TypeBox (or revise the claim)',
        depends: 'the re-run',
        spec: [
          'Compare AOT vs runtime and vs typia/TypeBox; if the target is missed, document why and update the architecture claim honestly.',
        ],
        tests: ['Guardrail test: AOT ops/s ≥ 5× runtime ops/s on assert/parse fixtures.'],
        accept: [
          'Acceptance target met and asserted, OR an honest written revision of the AOT claim.',
          'Closing this + prior subs fully resolves the epic.',
        ],
      },
    ],
  },
  {
    ...
    subs: [
      {
        t: 'Freeze spec: join grammar + golden SQL per dialect + aliasing',
        depends: 'none',
        spec: [
          'Define innerJoin/leftJoin/rightJoin + on() predicate grammar.',
          'Freeze golden SQL per dialect + table/column aliasing rules.',
        ],
      ...
      {
        t: 'Implement inner/left/right join + on() compilation',
        depends: 'the spec-freeze sub-issue',
      ...
      {
        t: 'Self-join + multi-join + aliasing',
        depends: 'join compilation',
      ...
      {
        t: 'Repository integration + E2E on real Postgres',
        depends: 'self/multi-join',
      ...
      {
        t: 'Re-run the DNF drizzle-bench join routes; confirm they now serve',
        depends: 'repository integration',
        spec: ['The previously-501 join routes serve 200 with correct rows; record throughput.'],
        tests: [
          'Harness check: /employee-with-recipient and /product-with-supplier return 200; k6 records throughput.',
        ],
        accept: [
          'Formerly-DNF join routes now served; RESULTS.md updated.',
          'Closing this + prior subs fully resolves the epic.',
        ],
      },
    ],
  },
  {
    ...
    subs: [
      {
        t: 'Freeze spec: aggregate/select-expression grammar + golden SQL + result typing',
        depends: 'none',
        spec: [
          'Define count/sum/avg/min/max + arithmetic select expressions, groupBy, having.',
          'Freeze golden SQL + computed-column result typing.',
        ],
        tests: ['Failing golden SQL tests for grouped aggregates + computed columns.'],
        accept: ['Committed SPEC.md.', 'Tests compile and FAIL.'],
      },
      {
        t: 'Implement aggregate functions + arithmetic select expressions',
        depends: 'the spec-freeze sub-issue',
      ...
      {
        t: 'Implement groupBy + having',
        depends: 'aggregate functions',
      ...
      {
        t: 'Repository integration (typed computed columns) + E2E on real Postgres',
        depends: 'groupBy/having',
      ...
      {
        t: 'Re-run the DNF aggregate routes; confirm 200 + correct aggregates',
        depends: 'repository integration',
        ...
        accept: [
          'Formerly-DNF aggregate routes served; RESULTS.md updated.',
          'Closing this + prior subs fully resolves the epic.',
        ],
      },
    ],
  },
  {
    ...
    subs: [
      {
        t: 'Freeze spec: FTS predicate grammar + per-dialect golden SQL + DNF map',
        depends: 'none',
        spec: [
          'Define whereMatch(column, term) → to_tsvector(col) @@ to_tsquery($1) for Postgres.',
          'Freeze the honest per-dialect FTS map (pg tsvector; sqlite FTS5; mysql MATCH…AGAINST; DNF where absent).',
        ],
      ...
      {
        t: 'Implement Postgres to_tsvector/@@/to_tsquery compilation',
        depends: 'the spec-freeze sub-issue',
      ...
      {
        t: 'Re-run /search-customer + /search-product; confirm they serve',
        depends: 'repository integration',
        ...
        accept: [
          'Formerly-DNF search routes served; RESULTS.md updated.',
          'Closing this + prior subs fully resolves the epic.',
        ],
      },
    ],
...
function body(epic, sub, _idx) {
  const L = [`Parent epic: #${epic.parent} (${epic.title})`, ''];
  L.push('## Goal', sub.t, '');
  L.push(
    '## Depends on',
    sub.depends === 'none' ? 'Nothing — spec-freeze starting point (TDD).' : `Previous: ${sub.depends}.`,
    '',
  );
...
for (const epic of EPICS) {
  ...
  const id = {};
  for (const n of [epic.parent, ...created])
    id[n] = graphql('query($o:String!,$r:String!,$n:Int!){repository(owner:$o,name:$r){issue(number:$n){id}}}', {
      o: OWNER,
      r: REPO,
      n,
