...
describe('owned architecture exceptions (#735)', () => {
  it('reports zero remaining debt and keeps the removed opaque stores absent', async () => {
  ...
  it('classifies the live findings exactly and preserves every owned finding in reports', async () => {
    ...
    expect(reports.flatMap(report => report.findings)).toHaveLength(0);

    const migration = renderGovernanceExceptionMigrationReport();
    expect(migration).toContain('- total legacy entries: 0');
    expect(migration).toContain('- total live exceptions: 0');
  ...
  it('publishes byte-identical focused exception reports through the aggregate snapshot', async () => {
    ...
    expect(Object.values(query?.reports ?? {}).flatMap(report => report.findings)).toHaveLength(0);
    expect(
    ...
    expect(snapshot.findings.filter(finding => finding.disposition === 'excepted')).toHaveLength(0);
  }, 90_000);
  ...
  it('is deeply frozen, deterministically ordered, and backed by existing evidence', () => {
    ...
    expect(new Set(GOVERNANCE_EXCEPTIONS.map(exception => exception.id)).size).toBe(0);
    expect(
