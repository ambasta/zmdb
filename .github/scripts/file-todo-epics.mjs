...
const EPICS = [
    ...
    subs: [
      {
        ...
        behavior:
          'Freeze the index/unique/partial/expression + check-constraint DSL grammar, its column metadata shape, and golden DDL per dialect (pg/mysql/sqlite).',
      },
      {
        kind: 'impl',
        title: 'Indexes & Constraints',
        behavior:
          'Implement the index/constraint builders + migration diff → DDL. Compose with existing table diffing.',
      },
      {
        kind: 'spec',
        title: 'Views (incl. materialized)',
        behavior:
          'Freeze view/materialized-view declaration grammar and golden CREATE/DROP VIEW DDL + refresh semantics.',
      },
      {
        kind: 'impl',
        title: 'Views (incl. materialized)',
        behavior: 'Implement view objects + diff → DDL; ensure derived read types are available.',
      },
      {
        kind: 'spec',
        title: 'Sequences',
        behavior: 'Freeze standalone sequence object grammar (start/increment/cache) and golden DDL.',
      },
      { kind: 'impl', title: 'Sequences', behavior: 'Implement sequence objects + diff → DDL.' },
      {
        ...
        behavior:
          'Freeze stored/virtual generated-column grammar and golden DDL + type-derivation rules (excluded from CreateDTO).',
      },
      {
        kind: 'impl',
        title: 'Generated columns',
        behavior: 'Implement generated columns in the DSL, type derivation, and diff → DDL.',
      },
      {
        kind: 'spec',
        title: 'Schemas / namespaces',
        behavior: 'Freeze multi-namespace grammar and qualified-identifier SQL rules.',
      },
      {
        kind: 'impl',
        title: 'Schemas / namespaces',
        behavior: 'Implement namespace qualification across query builder + migrations.',
      },
      {
        kind: 'spec',
        title: 'Row-Level Security (RLS)',
        behavior: 'Freeze RLS policy declaration grammar and golden CREATE POLICY / ENABLE RLS DDL.',
      },
      { kind: 'impl', title: 'Row-Level Security (RLS)', behavior: 'Implement RLS policy objects + diff → DDL.' },
      {
        kind: 'docs',
        title: 'Flip schema-object docs pages TODO → supported',
        pages: ['indexes-constraints', 'views', 'sequences', 'generated-columns', 'schemas-namespaces', 'rls'],
      },
    ],
  },
  {
    ...
    subs: [
      {
        ...
        behavior:
          'Freeze union/intersect/except grammar, type-merge rules for the row type, and golden SQL per dialect.',
      },
      { kind: 'impl', title: 'Set operations', behavior: 'Implement set-operation composition + result typing.' },
      {
        kind: 'spec',
        title: 'Batch API',
        behavior: 'Freeze the batch request/response contract and driver capability negotiation.',
      },
      {
        kind: 'impl',
        title: 'Batch API',
        behavior: 'Implement batched execution with single-roundtrip where supported; deterministic result order.',
      },
      {
        kind: 'docs',
        title: 'Flip set-operations + batch docs pages TODO → supported',
        pages: ['set-operations', 'batch'],
      },
    ],
  },
  {
    ...
    subs: [
      {
        ...
        title: 'Read replicas',
        behavior:
          'Freeze the routing contract (reads→replica, writes+tx→primary), the composition API over Driver, and failover policy.',
      },
    ...
    subs: [
      {
        ...
        behavior:
          'Freeze the custom-type contract (sqlType, tsType, serialize/deserialize), and how it flows into Entity/DTO derivation + validation + Ser/De.',
      },
      {
        kind: 'impl',
        title: 'Custom types & codecs',
        behavior: 'Implement custom-type registration + codec hooks across schema-core/query-compiler/aot-validator.',
      },
    ...
    subs: [
      {
        ...
        title: 'Seeding',
        behavior:
          'Freeze the seed API (per-schema count, seeded RNG, relation-aware ordering) and reproducibility contract.',
      },
      {
        kind: 'impl',
        title: 'Seeding',
        behavior: 'Implement the seeder on top of random<T> + repository create, respecting FK order.',
      },
  ...
  },
  {
    ...
    subs: [
      {
        kind: 'spec',
        title: 'Lifecycle events & subscribers',
        behavior:
          'Freeze the explicit event surface (before/after create/update/delete), subscriber registration, and the explicit-not-implicit guarantee.',
      },
      {
        kind: 'impl',
        title: 'Lifecycle events & subscribers',
        behavior: 'Implement the event dispatch around repository write methods.',
      },
      {
        kind: 'spec',
        title: 'Embeddables',
        behavior: 'Freeze embeddable value-object grammar, column-prefix mapping, and derivation/validation rules.',
      },
      {
        kind: 'impl',
        title: 'Embeddables',
        behavior: 'Implement embeddables in schema-core derivation + query mapping.',
      },
      {
        kind: 'spec',
        title: 'Inheritance mapping',
        behavior:
          'Freeze single-table (+optional class-table) inheritance grammar, discriminator handling, DDL, and derived-type union rules.',
      },
      {
        kind: 'impl',
        title: 'Inheritance mapping',
        behavior: 'Implement inheritance mapping + DDL + type derivation.',
      },
      {
        kind: 'docs',
        title: 'Flip entity-modeling docs pages TODO → supported',
        pages: ['lifecycle-hooks', 'embeddables', 'inheritance'],
      },
    ],
  },
  {
    ...
    subs: [
      {
        kind: 'spec',
        title: 'Framework integration contract',
        behavior:
          'Freeze the shared adapter contract (request→assert→handler→stringify→response) and per-framework surface for NestJS/Hono/tRPC/Express.',
      },
      {
        kind: 'impl',
        title: 'NestJS + Hono + tRPC + Express adapters',
        behavior: 'Implement the optional adapters/recipes with example handlers + E2E.',
      },
      {
        kind: 'docs',
        title: 'Flip framework-integrations docs page TODO → supported',
  ...
  },
  {
    ...
    subs: [
      {
        kind: 'spec',
        title: 'LLM function-calling contract',
        behavior:
          'Freeze the application()/parameters()/structuredOutput() surface, the schema mapping, and the lenient parse/coerce/validate + feedback contract.',
      },
      {
        kind: 'impl',
        title: 'LLM function-calling harness',
        behavior:
          'Implement schema generation + lenient parse/coerce/validate on top of the AOT validator + OpenAPI generator.',
      },
