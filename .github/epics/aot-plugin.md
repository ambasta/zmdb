...
zmdb currently runs its **runtime** validator (`is`/`equals` walking a `TypeDescriptor`), because `transformSource` is not wired as a build plugin.
...
Ship `@zmdb/aot-validator` as a **real TypeScript transformer build plugin** (ts-patch / ttypescript / unplugin), so `is<T>()` / `assert<T>()` / `validate<T>()` calls are replaced at build time with
inlined, allocation-free, monomorphic JS — no `TypeDescriptor` walk, no per-field function dispatch at runtime.
