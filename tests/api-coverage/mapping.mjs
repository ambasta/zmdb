...
export const kysely = {
  ...
  'schema > alter table > add unique constraint': 'emits database-owned unique and foreign-key constraints',
...
export const drizzle = {
  ...
  // --- schema declaration --------------------------------------------------
  'table config*': [
    'emits database-owned unique and foreign-key constraints',
...
export const mikroOrm = {
  ...
  naming: [
    ...
    'snake_cases the names every hand-rolled implementation gets wrong',
  ...
  dataloader: [
    ...
    'reuses a loaded id inside one scope without sharing the cached row object',
    'chunks a batch that would exceed the dialect parameter limit',
    'batches composite primary keys without crossing tuple boundaries',
    'coalesces declared relations across parents with their cardinality',
    'keeps relation results scope-local and returns fresh child rows',
  ],
  'joined-strategy': oos(
  ...
  'result-cache': [
    ...
    'serializes object parameters in stable key order',
    'expires a cached result after its TTL',
    'evicts by LRU at maxEntries',
    'invalidates by tag on a write to the table',
    'invalidates table and caller tags after every successful write',
    'invalidates a caller tag across repositories sharing a store',
    'does not cache anything when no cache option is given',
    'bypasses an existing cache in both directions when cache is false',
    'owns the default memory store per repository instance',
    'falls through to the database when the store throws, and reports once',
    'keeps database reads and writes available when store set or invalidation fails',
