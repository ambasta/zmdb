...
At the #618 baseline, six directories under `packages/` contain publishable manifests, and `.github/scripts/lib/publish-manifest.mjs` repeats those six in a hard-coded publication array:
...
This table is review evidence, not the canonical machine source. #622 creates the six catalog rows, assigns `docsOwner` and `consumer`, and makes later package additions or renames single catalog
