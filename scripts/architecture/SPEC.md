...
> #660, and the #710 AI ownership cutover. Issue #724 implements the canonical policy plus read-only discovery and graph APIs; #725 implements architecture-zone, ring and workspace-edge enforcement;
> and #727 implements package metadata and lockstep-manifest enforcement. #726 implements policy-driven runtime, tooling and optional-peer reachability; only the release verifier remains a later
> slice. The original measured baseline is commit `5adba11e` on 2026-09-05.
...
Issues #656, #682, #705, #647, #650, #706, #707, #708, #709, #662, #669, #691, #657, #658, #659, and #660 add `@zmdb/protobuf`, `@zmdb/client`, `@zmdb/ai`, `@zmdb/app`, `@zmdb/jobs`,
...
`@zmdb/transport-redis`. Issue #710 removed the temporary LangChain-to-schema-core edge. The current twenty-two manifests keep `1.0.0-alpha.4`, declare 39 direct non-dev workspace edges, and declare
14 peer dependencies: 8 optional peers plus required OpenTelemetry, React, grpc-js, NATS, RabbitMQ, and Redis peers confined to their selected integration packages.
...
The following object is normative. It constrains the current twenty-two catalog members, and the runtime-reachability gate verifies every present export and executable against it. Adding, removing or
...
`@zmdb/react`, `@zmdb/otel`, `@zmdb/transport-grpc`, `@zmdb/transport-nats`, `@zmdb/transport-rabbitmq`, and `@zmdb/transport-redis` reach their required peers under §5.4, so they do not use an
ordinary-runtime dependency allowance.
