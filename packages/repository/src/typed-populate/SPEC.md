...
- The columns the batched select matches on come from `resolveRelation(schema.ir, name)` in `@zmdb/schema-core`. `OneToMany<'orders', 'userId'>` names the target table and the foreign key;
...
- With `populate: ["orders"]`, the repository fetches the parents, then runs ONE batched `IN (...)` query per relation and attaches results via `attachPopulated` (to-many → `readonly Entity<Order>[]`,
  to-one → `Entity<User> | null`). The result type is `Populated<T, "orders">`.
