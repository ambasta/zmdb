...
interface Subscriber { on: LifecycleEvent; run: (ctx: unknown) => void | Promise<void>; }
class EventBus {
...
```ts
interface Embeddable { prefix: string; fields: readonly string[]; }
...
```ts
interface SingleTableInheritance { discriminator: string; map: Record<string, readonly string[]>; }
