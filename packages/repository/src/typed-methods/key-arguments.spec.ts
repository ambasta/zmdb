...
// is `../../SPEC.md` §2.1.
//
// `composite-pk.spec.ts` next door covers the happy path: a two-column key passed as
...
function findByIdFromRuntime(repository: object, id: unknown): Promise<unknown> {
  const method = Reflect.get(repository, 'findById');
  if (typeof method !== 'function') throw new TypeError('repository has no findById method');
  return Reflect.apply(method, repository, [id]);
}
...
describe('composite key refusals (frozen: repository/SPEC.md 2.1)', () => {
  ...
  it('names the table, the method and the missing column', async () => {
    ...
    await expect(half).rejects.toBeInstanceOf(IncompleteKeyError);
    await expect(half).rejects.toThrow('tenant_users.findById requires every key column; missing: userId');
    await expect(half).rejects.toMatchObject({ table: 'tenant_users', missing: ['userId'] });
  ...
  // call.
  it('lists every missing key column in key order', async () => {
  ...
  it('ignores an inherited property when checking a composite key', async () => {
    const { driver, calls } = recorder();
    const repo = new TenantUsersRepo(driver);
    const inherited = { tenantId: 't1' };
    Object.setPrototypeOf(inherited, { userId: 10 });

    const found = findByIdFromRuntime(repo, inherited);
    await expect(found).rejects.toBeInstanceOf(IncompleteKeyError);
    await expect(found).rejects.toThrow('tenant_users.findById requires every key column; missing: userId');
    expect(calls).toEqual([]);
  });

  // The method in the message is the one the caller called. `keyWhere` is the private helper
  ...
  it('names update and delete as themselves, not the shared helper', async () => {
    const { driver } = recorder();
    const repo = new TenantUsersRepo(driver);
  ...
  // out which columns.
  it('says what arrived and what the key is, for a non-object key', async () => {
    const { driver } = recorder();
    const repo = new TenantUsersRepo(driver);
...
describe('a keyless table (frozen: repository/SPEC.md 2.1)', () => {
  ...
  // `keyWhere` for the other two rules does not lose it.
  //
...
describe('a one-column key takes the value, not a record (frozen: repository/SPEC.md 2.1)', () => {
  ...
  // not tidiness. Before #608, the key helper built `{ [keyColumn]: id }` for a one-column key,
  // so the record form produced the nested `{ id: { id: 42 } }`, which `compileWhere` read as an
  // operator map, found no operator in, and emitted nothing for. The predicate did not become
  // wrong; it disappeared.
  //
  ...
  // single-column branch of `keyWhere` now refuses a non-scalar key.
...
describe('all key columns are pagination tie-breakers (frozen: repository/SPEC.md 2.1)', () => {
  // `list` orders by the key so that a page boundary is deterministic, and encodes the ordering
  // columns into the cursor. A first-column-only fallback gives a composite key ties and leaves
  // the cursor unable to say which side of one it was on.
  //
  // Before #411, both statements were measured as:
  ...
  it('orders and cursors by every key column, so no row is skipped between pages', async () => {
    const rows = [
    ...
    expect(calls[1]?.parameters).toEqual(['t1', 't1', 11]);
  });
});
