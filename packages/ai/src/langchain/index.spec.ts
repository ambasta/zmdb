...
const ADAPTER_ENTRIES = [
  join(ROOT, 'packages/ai/src/langchain/index.ts'),
...
describe('@zmdb/ai/langchain', () => {
  ...
  it('keeps the LangChain peer optional on @zmdb/ai and absent from the schema manifest', () => {
    const schema = readManifest('schema');
    const ai = readManifest('ai');

    for (const field of ['dependencies', 'peerDependencies', 'peerDependenciesMeta']) {
      expect(manifestField(schema, field)).not.toHaveProperty('@langchain/core');
    }

    expect(manifestField(ai, 'dependencies')).not.toHaveProperty('@langchain/core');
    expect(manifestField(ai, 'peerDependencies')).toMatchObject({ '@langchain/core': '^1.2.9' });
    expect(manifestField(ai, 'peerDependenciesMeta')).toMatchObject({ '@langchain/core': { optional: true } });
    expect(manifestField(ai, 'devDependencies')).toMatchObject({ '@langchain/core': '1.2.9' });
    expect(manifestField(ai, 'dependencies')).not.toHaveProperty('zod');
    expect(manifestField(ai, 'peerDependencies')).not.toHaveProperty('zod');
  });
});
