...
> **Status:** public package boundary implemented by issue #707 and physically cut over by #710. The manifest, root implementation, real LangChain tests and consumer fixture now use
> `@zmdb/ai/langchain` with no schema-core runtime dependency.

## 1. Responsibility

This package owns the LangChain-specific tool-object translation in `packages/ai/src/langchain/index.ts`. It accepts the provider-neutral schema/tool contract from `@zmdb/ai`, returns the structural
fields accepted by LangChain's structured-tool API, and exposes `@zmdb/ai/tool-runtime`'s adapter options.
...
The #710 ownership cutover moved:

- `adapters/langchain.ts` to `packages/ai/src/langchain/index.ts`; and
- `adapters/langchain.spec.ts` to `packages/ai/src/langchain/index.spec.ts`.

The shared adapter runtime moves to `@zmdb/ai/tool-runtime`; it is not copied as an undeclared cross-package source import.

Issue #707 moved the executable and type-level contract suites to this package; #710 replaced the temporary forwarder with the implementation and removed the old source without introducing a reverse
schema-core-to-AI edge.
