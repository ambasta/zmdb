...
This package owns the Vercel AI SDK tool-object translation in `packages/ai/src/vercel/index.ts`. It adapts a provider-neutral zmdb tool document to the SDK's branded `inputSchema` contract and uses
...
- `adapters/ai-sdk.ts` to `packages/ai/src/vercel/index.ts`;
- `adapters/ai-sdk.spec.ts` to `packages/ai/src/vercel/index.spec.ts`;
