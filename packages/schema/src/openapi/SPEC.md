...
> Status: **FROZEN** for TDD. Implementation (#64–#67) must satisfy this spec. Part of `@zmdb/schema` (module `src/openapi/`). Build-time only, no runtime reflection. Targets: Node 26+, ESM, TS 7.
...
Builds `components.schemas` keyed by PascalCase table name (`users` → `User`). Deterministic: same input → byte-identical output.
