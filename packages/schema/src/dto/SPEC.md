...
longer read a column map, so the parameter is the declaration and this document is keyed the same way `Entity<T>` is. Every shape here also re-exports from `@zmdb/schema/derive/query`, which is the
same type by a second name.
...
Recorded during the non-SQL target feasibility study (`@zmdb/sql`'s `src/targets/SPEC.md`), because both halves of it are load-bearing and neither was written down.
