...
On the type it belongs to, once, with a tag from `@zmdb/schema/tags`:
...
`packages/orm/src/tagged-to-ddl.spec.ts` follows the declaration through the real snapshot and asserts that all four root-dialect plans contain `REFERENCES`; Cockroach inherits the Postgres form,
while SingleStore refuses foreign keys explicitly. `tests/api-coverage/mapping.mjs` cites that shipped behavior.
