...
`);
  ...
  it('defineConfig is the identity function', async () => {
    ...
    expect(rootDefineConfig).toBe(contractDefineConfig);
    expect(productLoadConfig).toBe(canonicalLoadConfig);
  ...
  it('keeps the generated config validator current', () => {
    const result = codegen({
      project: join(ROOT, 'packages', 'compiler', 'tsconfig.codegen.json'),
      check: true,
    });
