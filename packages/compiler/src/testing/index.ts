  ...
  return withSession({ project: options.project }, session => {
    ...
    for (const file of files.toSorted()) {
      ...
      const exported = session.checker
        ...
        .toSorted((left: { name: string }, right: { name: string }) => left.name.localeCompare(right.name));
      ...
      if (options.onDiagnostics) options.onDiagnostics(diagnostics);
        ...
        throw new Error(
          `the reflection refused ${diagnostics.length} thing(s) in the configured schema files:\n` +
            diagnostics.map((one: ReflectDiagnostic) => `  ${one.path ? `${one.path}: ` : ''}${one.reason}`).join('\n'),
        );
      }
    }
    if (schemas.size === 0) {
    ...
    return [...schemas.values()].toSorted((left: CoreSchema<string>, right: CoreSchema<string>) =>
