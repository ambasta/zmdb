...
`../reflect/` turns a type into a `TypeIR`. This turns a `TypeIR` into JavaScript, so that
...
- `CALLEES` in `../transform/index.ts` includes all three message names and the two gRPC artifact names. With `toolFor`'s provider-document target, the complete list now has seventeen entries, and
