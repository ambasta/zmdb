...
import { tsTypeToTypeIR, emitCheckFromIR } from '../transform/index.js';
...
describe('TypeChecker Integration with TypeIR in Unplugin', () => {
  ...
  const routesFilePath = path.resolve('benchmarks/harness/framework/routes.ts');
  ...
  it('Requirement 1 & AC 1: extracts full structural type info for imported interfaces and generates static validation statements', () => {
    // UserCreate is an imported interface/type alias in routes.ts with fields name: string, email: string
    const src = 'const ok = is<UserCreate>(input);';
    const sourceFile = proj.program.getSourceFile(routesFilePath);

    const out = transformTypeChecks(src, { sourceFile, checker, id: routesFilePath });
    const n = norm(out);
  ...
  it('Requirement 3 & AC 2: type aliases imported across workspace package boundaries resolve via path configurations', () => {
    ...
    const transformed = plugin.transform(sampleCode, routesFilePath);
  ...
  it('Requirement 4 & AC 4: non-imported inline primitive validations continue to transform without added compilation latency', () => {
    ...
    for (let i = 0; i < iterations; i++) {
      transformTypeChecks(inlineSrc, { sourceFile, checker, id: routesFilePath });
    }
    const duration = performance.now() - start;

    const out = transformTypeChecks(inlineSrc, { sourceFile, checker, id: routesFilePath });
