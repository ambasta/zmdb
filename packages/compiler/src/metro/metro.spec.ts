...
describe('the Metro build path', () => {
  ...
  it(
    ...
    async () => {
      ...
      const version = (require(join(ROOT, 'packages', 'compiler', 'package.json')) as { version: string }).version;
  ...
  it(
    ...
    async () => {
      ...
      // Keep the disposable project outside the repository tree. The compiler lint
      // dogfood test scans `fixtures/` concurrently in the complete suite, and a
      // project removed between oxlint's directory walk and file read is a false
      // repository error rather than a compiler failure.
      const cli = mkdtempSync(join(SCRATCH, 'cli-route-'));
      try {
        mkdirSync(join(cli, 'src'));
        symlinkSync(join(ROOT, 'node_modules'), join(cli, 'node_modules'), 'dir');
  ...
  it('reuses the cache-key compiler session when Metro transforms in the config process', () => {
    ...
    const session = require(join(ROOT, 'packages/compiler/src/reflect/session.ts'));
