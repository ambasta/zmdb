...
import { type PropertyIR, type SchemaIR, type ShapeIR, type TypeIR } from '@zmdb/schema/ir';
...
export function transformFile(fileName: string, code: string, context: TransformContext): TransformResult {
  ...
  if (sites.length === 0) {
    const out = transformCode(code, { id: fileName });
  ...
  let out = transformCode(rewriter.text, { id: fileName });
