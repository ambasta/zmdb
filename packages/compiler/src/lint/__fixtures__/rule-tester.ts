...
export async function loadLintModule(): Promise<LintModule> {
  if (!isLintModule(lintModule)) throw new Error('@zmdb/compiler/lint has an invalid plugin shape');
  return lintModule;
}
...
export async function runRuleCase(ruleName: string, tests: RuleTester.TestCases): Promise<void> {
  ...
  if (rule === undefined) throw new Error(`@zmdb/compiler/lint does not export rule "${ruleName}"`);
