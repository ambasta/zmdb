// What `@zmdb/compiler` project compilation does to the source file it rewrites.
//
...
describe('a session the caller owns', () => {
  // The plugin has taken a borrowed session from the start; the original no-bundler path did
  // not, and a tool that has the project loaded already should not pay to load it again — that
  // is REQ-TF-11 pointed at the caller rather than at the file loop. It is also what
  // `verify:build-budget` needs in order to watch a build from outside it.
