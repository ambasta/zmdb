...
> Part of `@zmdb/schema-core` (module `src/tags/`). Types only — the module has no runtime export at all, and `erasure.spec.ts` enforces that. Implements REQ-TF-1 … REQ-TF-3. Design:
...
**Settled — plan D6.** `@zmdb/aot-validator` exports a _runtime_ vocabulary for the same five constraints, and a `ValidationRule` spells them with the IR's own keyword
(`{ kind: 'minimum', value: n }`).
