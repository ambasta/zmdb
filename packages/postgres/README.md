...
`down` from `@zmdb/migrations`; transactional behavior follows the capability table below. Package-local migration and integration tests cover application, rollback and connection cleanup.
...
Package-local driver and PostgreSQL integration tests cover the database behavior. The [PostgreSQL consumer](../../fixtures/database-postgres) adds database-specific capability and refusal checks.
