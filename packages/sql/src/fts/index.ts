...
import { type CompiledQuery, type Operator, type QueryCompilerOptions } from '../index.js';
...
type Predicate =
  | { kind: 'match'; col: string; value: unknown }
  | { kind: 'cmp'; col: string; op: Operator; value: unknown };
interface State {
  table: string;
...
export interface FtsSelect {
  ...
  where(col: string, op: Operator, value: unknown): FtsSelect;
