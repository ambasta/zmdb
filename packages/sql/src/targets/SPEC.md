...
> Part of `@zmdb/sql`. Spec-only, like `../dialects/`: this directory exists to hold a decision, and the decision is that neither target ships. Read `../../SPEC.md` §5f for the seam this measures and
> `../dialects/SPEC.md` for the SQL matrix, which is a different question.
...
> `@zmdb/schema/dto` and drive the builder through two **structural** interfaces, `WhereTarget` and `OrderTarget`, whose methods are `where(col, op, value)` and `orderBy(col, dir)` and which name no
> SQL and no `CompiledQuery`. A non-SQL target that implements those interfaces and returns its own compiled shape is driven by the existing code, unchanged, with no type parameter anywhere and no
...
`@zmdb/schema` has the same `{ compile(): CompiledQuery }` arm, so `WhereDTO`'s `exists`/`notExists` and every `FieldOps` operator can carry one.
...
```ts
// Not a new interface — this is what `@zmdb/schema/dto` already drives.
