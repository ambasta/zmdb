...
During the server-package migration, only the concrete Redis adapter retains its named `/microservices/*` entry point. Typed gRPC servers and clients ship from `@zmdb/transport-grpc`, core NATS ships
from `@zmdb/transport-nats`, and RabbitMQ ships from `@zmdb/transport-rabbitmq`; none of their old web subpaths forwards. The transport-neutral strategy, dispatcher, decorators, and typed clients live
at `@zmdb/app/messaging`. Queues, workers, scheduling, leases, and the SQLite memory backend live in `@zmdb/jobs`; the removed web paths do not forward.
...
Redis remains an optional peer reached only through its named microservices subpath. Install each selected transport package with its required peer.
