...
import type { RouteOptions } from '../pipeline/index.js';
import { Controller, Delete, Get, Post } from '../routing/index.js';
import { toOpenApi, type OpenApiDocument, type OpenApiOptions } from './index.js';
...
//
// `it.fails` for every frozen claim, with the current output recorded above it, and plain `it` where
// a claim holds today and the implementation slice could break it while making a red one pass. See
// `../../../query-compiler/src/migrations/composite-keys.spec.ts` for why `it.fails` and not `.skip`
// or a stub. `./security.type-test.ts` carries §S9 item 2 and every claim about a type.
//
...
// the recorded actuals say: today every one of the options below is accepted and silently ignored, so
// an application that adopts this surface before #575 lands gets a document that describes none of
// its security and no error telling it so.
//
...
// ---------------------------------------------------------------------------
//
// Only the widening, intersected with the real exported types, and passed through the one boundary
// function below. All of it goes away in the slice that lands §S1-§S8 for real.

/** §S3: a guard that describes what it enforces. */
type AwareGuard = Guard & { readonly enforces: { readonly scheme: string; readonly scopes: readonly string[] } };

/** §S2. */
type FrozenRequirement = Readonly<Record<string, readonly string[]>>;

/** §S2 and §S8: the per-handler record `router.register` already accepts, widened. */
...
/**
 * §S2: `toOpenApi`'s options, widened.
 *
 * `securitySchemes` is `unknown`-valued here and not the six-arm union: the runtime claim in §S9
 * item 1 is that a declaration round-trips into the document unchanged, and the arms' shapes are
 * `./security.type-test.ts`'s to pin. Two files, one home per claim.
 */
type FrozenOptions = OpenApiOptions & {
  readonly securitySchemes?: Readonly<Record<string, unknown>>;
  readonly routes?: Readonly<Record<string, Readonly<Record<string, FrozenRouteOptions>>>>;
 ...
 */
const SCHEMES: Readonly<Record<string, unknown>> = {
...
describe('guard-derived security (frozen: openapi/SPEC.md S1-S8)', () => {
  ...
  it.fails('refuses to generate a route with neither guards nor a public marker', () => {
  ...
  // freeze an invention.
  //
  // actual today: expected [Function] to throw an error
  it.fails('refuses to generate a guarded route whose guards cannot declare', () => {
  ...
  //
  // actual today: expected [Function] to throw an error
  it.fails('refuses an explicit requirement that omits a scheme its guards enforce', () => {
  ...
  //
  // actual today: expected [Function] to throw an error
  it.fails('refuses an explicit requirement that drops a scope its guards enforce', () => {
  ...
  //
  // actual today: expected [Function] to throw an error
  it.fails('refuses an explicit requirement naming a scheme that was never declared', () => {
  ...
  //
  // actual today: expected [Function] to throw an error
  it.fails('refuses a guard enforcing a scheme that was never declared', () => {
  ...
  //
  // actual today: no security key
  it.fails('accepts an explicit requirement that adds a scheme the framework cannot see', () => {
  ...
  //
  // actual today: no security key
  it.fails('derives one requirement object from a single declaring guard', () => {
  ...
  //
  // actual today: no security key
  it.fails('merges two guards on one scheme into a sorted union of their scopes', () => {
  ...
  //
  // actual today: no security key
  it.fails('lists a scope two guards both enforce exactly once', () => {
  ...
  //
  // actual today: expected undefined to deeply equal [ { bearerAuth: [], apiKey: [] } ]
  it.fails('emits one requirement object for two schemes rather than one object each', () => {
  ...
  //
  // actual today: expected undefined to strictly equal { securitySchemes: { …(13) } }
  // - there is no `components` key at all.
  it.fails('round-trips every declared scheme into components.securitySchemes', () => {
  ...
  //
  // actual today: get,post | get=absent post=absent
  it.fails('marks the deprecated operation and nothing else on its path', () => {
  ...
  // documented example call `toOpenApi` with neither `routes` nor `securitySchemes`. This is the
  // assertion that breaks if strictness is applied to a call that has not opted in at all.
