...
// @ts-expect-error frozen (SPEC.md S4): `Public` and `isPublic` sit beside `getRoutes`.
...
//
// @ts-expect-error frozen (SPEC.md S1/S2/S3): the four security type names are exported from here.
import type { OAuthFlows, SecurityAwareGuard, SecurityRequirement, SecurityScheme, toOpenApi } from './index.js';
...
// @ts-expect-error frozen (SPEC.md S1): the exported union is 3.1's five types across six arms.
export type _SchemeShape = Expect<Equal<SecurityScheme, FrozenSecurityScheme>>;

// @ts-expect-error frozen (SPEC.md S1.3): the exported flows union requires at least one flow.
...
//
// @ts-expect-error frozen (SPEC.md S1): the returned document gains an optional `components`.
...
//
// @ts-expect-error frozen (SPEC.md S6): four document keys, and `security` is not one of them.
...
type FrozenOptionKeys = 'info' | 'schemas' | 'securitySchemes' | 'routes' | 'strictSecurity';

// @ts-expect-error frozen (SPEC.md S2): five options, and `routes` is one of them.
...
//
// @ts-expect-error frozen (SPEC.md S2): `routes` is that record, holding real `RouteOptions`.
...
//
// @ts-expect-error frozen (SPEC.md S4): `strictSecurity` is an optional boolean, defaulting true.
...
// @ts-expect-error frozen (SPEC.md S2): the scheme declarations, by name.
...
// @ts-expect-error frozen (SPEC.md S2/S8): `RouteOptions` gains guards, an override and deprecated.
...
//
// @ts-expect-error frozen (SPEC.md S2): `guards` on the per-handler record the router accepts.
...
// @ts-expect-error frozen (SPEC.md S5): the override is an array of requirement objects.
export type _SecurityOverride = Expect<Equal<RouteOptions['security'], readonly SecurityRequirement[] | undefined>>;

// @ts-expect-error frozen (SPEC.md S2): a requirement object maps a scheme name to its scopes.
...
//
// @ts-expect-error frozen (SPEC.md S8): `deprecated` is `true`, never `boolean`.
...
// @ts-expect-error frozen (SPEC.md S3): the exported guard adds `enforces` and nothing else.
