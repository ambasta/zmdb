...
describe('the platform under these tests', () => {
  ...
  it('has HMAC-SHA256 and base64url without node:crypto', async () => {
    ...
    const toB64 = (bytes: Uint8Array): string =>
      typeof (bytes as { toBase64?: unknown }).toBase64 === 'function'
        ? (bytes as { toBase64: (opts?: object) => string }).toBase64({ alphabet: 'base64url', omitPadding: true })
        : globalThis
            .btoa(String.fromCharCode(...bytes))
            .replace(/\+/g, '-')
            .replace(/\//g, '_')
            .replace(/=+$/, '');
    const encoded = toB64(mac);
    ...
    const againEncoded = toB64(again);
