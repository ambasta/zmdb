...
describe('the platform under these tests', () => {
  ...
  it('has HMAC-SHA256 and base64url without node:crypto', async () => {
    ...
    const encodeBase64Url = (bytes: Uint8Array) => {
      if (typeof (bytes as { toBase64?: (opts: unknown) => string }).toBase64 === 'function') {
        return (bytes as { toBase64: (opts: unknown) => string }).toBase64({
          alphabet: 'base64url',
          omitPadding: true,
        });
      }
      ...
      for (let i = 0; i < bytes.length; i += 3) {
        ...
        res += base64UrlChars[b0 >> 2];
        res += base64UrlChars[((b0 & 3) << 4) | (b1 >> 4)];
        if (i + 1 < bytes.length) res += base64UrlChars[((b1 & 15) << 2) | (b2 >> 6)];
        if (i + 2 < bytes.length) res += base64UrlChars[b2 & 63];
      }
      return res;
    };
    const encoded = encodeBase64Url(mac);
    ...
    expect(encodeBase64Url(again)).toBe(encoded);
