...
// contract-aware router, #682 supplies the runtime, #683 supplies the pure-IR
// OpenAPI projection, and #684 supplies deterministic client generation.

type GenerateClient = (contract: HttpContractIR) => string;

const projectOpenApi = toOpenApi;
const generateClient: GenerateClient = contract => generateHttpClient(contract).source;
...
describe('the shared HTTP contract convergence fixture', () => {
  ...
  it('OpenAPI and client generation read the same operation object', () => {
  ...
  it('changing a route changes both outputs', () => {
  ...
  // The generator accepts HttpContractIR only. Reflect.apply is deliberate: it
  // exercises the untyped JavaScript boundary without weakening the TypeScript
  // signature merely to make an invalid call compile.
  it('cannot generate a client from an OpenAPI document', () => {
  ...
  it('does not include credentials in generated source', () => {
  ...
  it('emits byte-identical output twice', () => {
