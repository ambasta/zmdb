// @zmdb/web — command applications (epic #497, spec ./SPEC.md).
// Commands use the same module compilation, container and lifecycle as createApp,
// with argv validation at the terminal boundary and no runtime reflection.
...
import { appCompilationOf } from '../app/runtime.js';
...
const COMMAND = Symbol('zmdb.web.command');
...
/**
 * Compile a root module through `createApp`, then dispatch its declared commands.
 * The returned app never calls `process.exit`; callers own `process.exitCode`.
 */
export function createCommandApp(rootModule: ModuleClass): CommandApp {
  const app = createApp(rootModule);
  const compiled = appCompilationOf(app);
  if (compiled === undefined) {
    throw new Error('@zmdb/web: command application could not read its compiled module graph');
  }
  const commands = registerCommands(compiled.commands);

  return {
    container: app.container,
    init: () => app.init(),
    run: (argv = process.argv.slice(2)) => runCommand(commands, argv),
    [Symbol.asyncDispose]: () => app[Symbol.asyncDispose](),
  };
}
...
function registerCommands(instances: readonly object[]): readonly RegisteredCommand[] {
  ...
  for (const instance of instances) {
    ...
    if (definition === undefined) {
      throw new Error(`@zmdb/web: ${instance.constructor.name} is listed in commands but has no @Command declaration`);
    }
    if (!isRunnableCommand(instance)) {
      throw new Error(`@zmdb/web: command "${definition.name}" has no run(args) method`);
    }
    if (definition.name.length === 0) {
      throw new Error('@zmdb/web: a command name cannot be empty');
    }
    if (names.has(definition.name)) {
      throw new Error(`@zmdb/web: duplicate command name "${definition.name}"`);
    }
...
function prepareCommand(instance: RunnableCommand, definition: CommandDef<unknown>): RegisteredCommand {
  ...
  for (const positional of positionals) {
    if (seenPositionals.has(positional)) {
      throw new Error(`@zmdb/web: command "${definition.name}" repeats positional "${positional}"`);
    }
    seenPositionals.add(positional);
    if (definition.args !== undefined && !propertyByName.has(positional)) {
      throw new Error(
        `@zmdb/web: command "${definition.name}" names positional "${positional}" but its args type has no such property`,
      );
    }
  }
  ...
  for (const property of properties) {
    if (optionNames.has(property.option)) {
      throw new Error(`@zmdb/web: command "${definition.name}" maps more than one property to --${property.option}`);
    }
...
function nestedArgument(command: string, property: string): Error {
  return new Error(`@zmdb/web: command "${command}" argument "${property}" has a nested shape; argv must be flat`);
}
