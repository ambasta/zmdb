// @zmdb/web — typed command bus (epic #591, spec ./SPEC.md).
...
function callable(record: object, key: string, kind: string): Function {
  ...
  if (typeof value !== 'function') {
    throw new Error(`@zmdb/web: command "${key}" has no callable ${kind}`);
  }
  return value;
}
...
export function createCommandBus<M extends CommandMap>(
  ...
  for (const command of Object.keys(handlers)) {
    ...
    if (!installed) {
      throw new Error(`@zmdb/web: could not install command "${command}"`);
    }
  }
  return bus;
}
