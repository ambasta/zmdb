...
describe('the zmdb CLI boundary', () => {
  ...
  it('declares the sole zmdb bin at @zmdb/cli src/bin.ts', () => {
    const record: { bin?: string | Record<string, unknown> } = JSON.parse(
      readFileSync(join(ROOT, 'packages', 'cli', 'package.json'), 'utf8'),
    );
    expect(Reflect.get(ZMDB_MANIFEST, 'bin')).toBeUndefined();
    const target = typeof record.bin === 'string' ? record.bin : record.bin?.['zmdb'];
    expect(target).toBe('./src/bin.ts');
  });
