...
describe('installed CLI qualification', () => {
  it('installs one CLI-owned bin and the stable product facade from real npm tarballs', async () => {
    await runCase('T01');
  }, 600_000);
  it('runs the complete installed CLI workflow without source shortcuts', async () => {
    await runCase('T26');
  }, 600_000);
});
