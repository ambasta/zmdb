...
export async function createReplSession(rootModule: ModuleClass, options: ReplSessionOptions): Promise<ReplSession> {
  ...
  try {
    ...
    try {
      ...
      throw new SuppressedErrorCtor(cleanupError, error, 'REPL operation and application cleanup failed');
    }
    throw error;
  }
    ...
    return {
      ...
      [Symbol.asyncDispose]: async () => {
        ...
        try {
          ...
          if (!outcome.ok)
            throw new SuppressedErrorCtor(cleanupError, outcome.error, 'REPL close and application cleanup failed');
          throw cleanupError;
        }
