...
import { down, driverMigrationConnection, status, up } from '@zmdb/migrations';
...
function isSnapshotableSchema(val: unknown): val is SnapshotableSchema {
  return Boolean(val && typeof val === 'object' && 'table' in val && 'columns' in val);
}
...
export async function main(argv = process.argv.slice(2), opts?: CliOptions): Promise<string> {
  ...
  const dialect = flags.dialect || 'sqlite';
  ...
  switch (command) {
    case 'generate': {
      ...
      if (flags.schema) {
        ...
        const mod: Record<string, unknown> = await import(pathToFileURL(schemaPath).href);
    ...
    case 'status': {
      ...
      const conn = driverMigrationConnection(driver);
