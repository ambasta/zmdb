...
import { loadConfig, type ResolvedConfig } from '@zmdb/compiler/config';
import { withSession } from '@zmdb/compiler/reflect';
...
describe('zmdb client generate', { timeout: 30_000 }, () => {
  ...
  it('watch invalidates only the actual contract dependency set', async () => {
    ...
    await withSession({ project: config.project }, async session => {
