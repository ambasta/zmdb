...
async function loadStudioApp(): Promise<StudioAppModule> {
  ...
  if (!isStudioAppModule(loaded)) {
    throw new Error('packages/cli/src/studio/index.ts does not export createStudioApp');
  }
  return loaded;
}
...
async function loadStudioCommand(): Promise<StudioCommandModule> {
  ...
  if (!isStudioCommandModule(loaded)) {
    throw new Error('packages/cli/src/commands/studio.ts does not export runStudio');
  }
  return loaded;
}
