# `@zmdb/app` — dependency injection SPEC
...
`@zmdb/web` consumes the public app entry; it does not retain `./di` or a forwarding file. The cohesive facade is a separate publish slice, so this move leaves one direct declaration owner.
