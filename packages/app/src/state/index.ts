...
// goes through a typed factory. No reflection.
...
/**
 * Define a domain state. `create` intentionally brands an already typed base
 * value. Validate unknown input with a generated base-type witness first; that
 * validation does not establish transition history. The brand exists only in
 * the type system, so `create` is an identity at runtime.
 */
