...
describe('@zmdb/app/otel', () => {
  ...
  it('maps every span kind, remote parent, tracestate and rename through the real SDK', async () => {
    ...
    try {
      const sdkTracer = provider.getTracer('@zmdb/app/otel/adapter-test');
  ...
  it('exports driver spans as clients and message spans as consumers with both W3C headers', async () => {
    ...
    try {
      const observability = fromOpenTelemetry({ tracer: provider.getTracer('@zmdb/app/otel/framework-test') });
