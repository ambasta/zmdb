...
Part of `@zmdb/app`; the `./cqrs` subpath ships a **command** bus only. The query bus, event sourcing and sagas are refused, each with a reason, in §6 — and recording those as decisions rather than
...
`ms` is from the global `performance.now()`, matching `../../../web/src/bench/index.ts`. `node:perf_hooks` is imported nowhere in this project and is not introduced here.
...
It remains independent of HTTP middleware and jobs. `@zmdb/web/cqrs` is deleted with no compatibility export; the cohesive facade is a separate publish slice.
