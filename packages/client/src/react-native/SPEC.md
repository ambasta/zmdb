# `@zmdb/client/react-native` — native lifecycle and connectivity SPEC
...
`@zmdb/client/react-native` is ESM-only, has `sideEffects: false`, depends only on `@zmdb/client` and `@zmdb/client/react`, and declares React 19.2 and React Native 0.87 as required peers. It is
opt-in and is not re-exported by `@zmdb/core`.
...
by `@zmdb/client/react`.

`@zmdb/client/react` exposes a per-provider `ZmdbReactRequestLifecycle` registration seam. The native provider passes that seam to the React provider, so AppState aborts the actual query and mutation
...
1. builds and packs `@zmdb/client`, `@zmdb/client/react`, and `@zmdb/client/react-native`;
