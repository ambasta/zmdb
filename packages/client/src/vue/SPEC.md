# `@zmdb/client/vue` — Vue plugin and composables SPEC
...
`@zmdb/client/vue` is ESM-only and has:
...
- `src/packed-consumer.spec.ts`, which builds and packs `@zmdb/client` and `@zmdb/client/vue`, installs only those tarballs plus Vue into a temporary project, typechecks without workspace paths,
  bundles a browser entry, runs browser and SSR entries, and executes all common conformance cases.
