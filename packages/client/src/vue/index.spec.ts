...
describe('@zmdb/client/vue native plugin and composables (#693)', () => {
  ...
  it('reports how to install a missing Vue plugin', () => {
    ...
    expect(() => app.runWithContext(() => bindings.useZmdbClient())).toThrow(
      '@zmdb/client/vue client is unavailable; install createZmdbPlugin(client) on the current Vue app',
    );
  });
