...
import type { ClientRuntime } from '../index.js';
...
function cancellation(action: string): Error {
  const error = new Error(`@zmdb/client/vue ${action}`);
...
function requireScope(primitive: string): void {
  if (getCurrentScope() === undefined) {
    throw new Error(`@zmdb/client/vue ${primitive} requires an active Vue effect scope or component setup`);
  }
}

export function createZmdbVue<Client extends object = ClientRuntime>(
  bindingName = '@zmdb/client/vue',
