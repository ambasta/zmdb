# `@zmdb/client/svelte` — typed context and stores specification
...
`@zmdb/client/svelte` is an ESM-only integration package with one root export. It depends inward on `@zmdb/client`, requires `svelte@>=5.57.0 <6.0.0` as a peer, and has no server, ORM, database,
cache, transport, generated-code, or competing HTTP dependency.
