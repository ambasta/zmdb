...
export function createQueryStore<Client, Input, Output>(
  ...
  const launch = async (selectedInput: Input, clearData: boolean, reason: string): Promise<void> => {
    if (destroyed) throw new SvelteAdapterError('@zmdb/client/svelte query store is destroyed');
    if (!active)
      throw new SvelteAdapterError('@zmdb/client/svelte query store is inactive; subscribe before refreshing');
  ...
  const startForInput = (selectedInput: Input): void => {
    ...
    void launch(
      ...
      changed,
      changed
        ? '@zmdb/client/svelte query superseded by an input-store change'
        : '@zmdb/client/svelte query restarted for a new subscription',
    ).catch(() => undefined);
  };
  ...
  const store: SvelteQueryStore<Output> = {
    subscribe(run, invalidate) {
      ...
      return () => {
        ...
        subscribers -= 1;
        if (subscribers === 0) {
          deactivate('@zmdb/client/svelte query cancelled after its final subscriber left');
        }
      };
    },
    refresh() {
      if (!active) {
        return Promise.reject(
          new SvelteAdapterError(
            destroyed
              ? '@zmdb/client/svelte query store is destroyed'
              : '@zmdb/client/svelte query store is inactive; subscribe before refreshing',
          ),
        );
      }
      if (currentInput === undefined) {
        return Promise.reject(new SvelteAdapterError('@zmdb/client/svelte query store has no current input'));
      }
      return launch(currentInput.value, false, '@zmdb/client/svelte query superseded by refresh');
    },
    destroy() {
      if (destroyed) return;
      destroyed = true;
      deactivate('@zmdb/client/svelte query cancelled because its owner was destroyed');
    },
  };
