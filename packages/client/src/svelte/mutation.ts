...
export function createMutationStore<Client, Input, Output>(
  ...
  const store: SvelteMutationStore<Input, Output> = {
    subscribe(runSubscriber, invalidate) {
      ...
      return () => {
        ...
        if (subscribers === 0 && requests.size > 0) {
          cancelActive('@zmdb/client/svelte mutation cancelled after its final subscriber left');
        }
      };
    },
    async mutate(input) {
      if (destroyed) throw new SvelteAdapterError('@zmdb/client/svelte mutation store is destroyed');
    ...
    destroy() {
      if (destroyed) return;
      destroyed = true;
      cancelActive('@zmdb/client/svelte mutation cancelled because its owner was destroyed');
    },
  };
