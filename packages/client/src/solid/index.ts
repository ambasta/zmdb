...
import type { CallOptions } from '../index.js';
...
function requireOwner(primitive: string): void {
  if (getOwner() === null) {
    throw new Error(`@zmdb/client/solid ${primitive} must be created under a Solid owner`);
  }
}
...
export function createZmdbSolid<Client>(): ZmdbSolidBindings<Client> {
  ...
  function useClient(): Client {
    ...
    if (client === undefined) throw new Error('@zmdb/client/solid client is unavailable outside its Provider');
    return client;
  }
  ...
  function query<Input, Output>(
    ...
    const [data, actions] = createResource<Output, Input>(readSource, input => {
      ...
      controller?.abort(new Error('@zmdb/client/solid query superseded'));
    ...
    onCleanup(() => {
      generation++;
      controller?.abort(new Error('@zmdb/client/solid query owner disposed'));
  ...
  function mutation<Input, Output>(run: MutationRunner<Client, Input, Output>): SolidMutation<Input, Output> {
    ...
    onCleanup(() => {
      ...
      for (const controller of controllers) {
        controller.abort(new Error('@zmdb/client/solid mutation owner disposed'));
      }
      controllers.clear();
    });
    ...
    return {
      ...
      pending,
      mutate(input) {
        if (!active) return Promise.reject(new Error('@zmdb/client/solid mutation owner is disposed'));
