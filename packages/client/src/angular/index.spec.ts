...
function angularExpectation() {
  ...
  if (expectation === undefined) throw new Error('missing @zmdb/client/angular package expectation');
  return expectation;
}
...
describe('@zmdb/client/angular native package contract (#692)', () => {
  ...
  it('importing the package does not require HttpClient', () => {
    const manifest = JSON.parse(readFileSync(join(ROOT, 'packages/client/package.json'), 'utf8')) as {
