...
Install the selected database package separately when planning dialect-specific SQL or connecting to a database, for example `@zmdb/sqlite@alpha`. The application supplies the driver or connection.
...
For a command workflow, install `@zmdb/cli@alpha` with `typescript@^7.0.2` and use its `zmdb generate`, `migrate`, `rollback`, `status`, `pull` and `embed` commands. The CLI loads project
