...
| An index expression or access method    | the same — needed for §1.1 of `../../../query-compiler/src/schema-objects/` |
...
- `table_info.pk` is a **1-based ordinal**, not a flag, which is how a composite key's _order_ is recovered — and order is normative per `../SPEC.md` §1.1, so this is the only dialect where the key
  order comes free.
...
sorted by name (`../SPEC.md` §1), so ordinal position is not available at that point — and carrying it into the snapshot would be worse than losing it: a column's ordinal is a fact about physical
layout that no zmdb DDL controls, since a column added by a migration lands last, so recording it would make a diff report a change every time two databases reached the same schema by different
routes.
...
The generic catalog protocols, common row validation, drift operation, selection rules and declaration emitter live at `@zmdb/migrations/introspect` and `@zmdb/migrations/declarations`.
Database-specific readers still move to the database packages frozen in §11.

Accordingly, §11.2's `index.ts` public result/protocol owner is `@zmdb/migrations/introspect`. `emit.ts` and `tagged-property.ts` live under the declarations subpath, which owns the sole `oxfmt` edge.
The old query-compiler introspection subpath does not resolve.
