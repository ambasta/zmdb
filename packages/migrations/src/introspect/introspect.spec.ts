...
import { type ChangeOp, type ColumnSnapshot, type SchemaSnapshot, type TableSnapshot } from '../index.js';
...
async function frozenExport<T>(name: string): Promise<T> {
  ...
  if (typeof value !== 'function') {
    throw new Error(`@zmdb/migrations exports no "${name}" (frozen: introspect/SPEC.md 1)`);
  }
  return value as T;
}
...
describe('catalog input boundary', () => {
  it('introspects recorded PostgreSQL, MySQL, SQLite and MSSQL catalogs into the same normalized snapshots', () => {
    ...
    const recordedCatalogs: Readonly<Record<Dialect, FrozenSchemaSnapshot>> = {
      postgres: emptySnapshot([
        {
          ...emptyTable('accounts', [
            {
              name: 'active',
    ...
    for (const dialect of ['postgres', 'mysql', 'sqlite', 'mssql'] as const) {
      expect(normalizeDriftSnapshot(recordedCatalogs[dialect], 'live', { dialect }), dialect).toEqual(expected);
    }
  });
