...
import { postgresDriver as pgDriver, type PgQueryable } from '@zmdb/postgres';
...
describe('migration runner E2E (real SQLite connection)', () => {
  ...
  it('applies, reports, rolls back and checksum-verifies a real SQLite migration ledger', async () => {
    ...
    await expect(status(adapter, [original])).resolves.toEqual([
      { version: original.version, name: original.name, applied: true },
    ]);
    ...
    await expect(down(adapter, [original])).resolves.toBe(original.version);
    await expect(status(adapter, [original])).resolves.toEqual([
      { version: original.version, name: original.name, applied: false },
    ]);
  });

  it('preserves non-transactional DDL warnings and transactional driver pinning', async () => {
    ...
    const mssqlConnection = driverMigrationConnection(
      {
        dialect: mssql,
        execute: async () => [],
      },
    ...
    await expect(mssqlConnection.transaction?.(async () => undefined)).rejects.toThrow(
...
describe('Snapshot-Bound Migrations Validation', () => {
  ...
  let boundConn: MigrationConnection;

  beforeEach(() => {
    database = new DatabaseSync(':memory:');
    boundConn = sqliteMigrationConn(database);
  });

  function boundTableInfo(): string[] {
  ...
  it('validates snapshot compatibility and executes database changes for snapshot-bound migrations', async () => {
    ...
    const done = await up(boundConn, boundMigrations, { targetSnapshot: snapV1 });
    expect(done).toEqual([1]);
    expect(await boundConn.appliedVersions()).toEqual([1]);
    expect(boundTableInfo()).toEqual(['id']);
  });
  ...
  it('halts execution before database modification when a migration conflicts with its target snapshot contract', async () => {
    ...
    try {
      await up(boundConn, [conflictingMigration], { targetSnapshot: snapV2 });
    } catch (err) {
    ...
    expect(await boundConn.appliedVersions()).toEqual([]);
  });
  ...
  it('existing migrations without snapshot references execute normally without behavior changes', async () => {
    ...
    const done = await up(boundConn, legacyMigrations);
    expect(done).toEqual([1]);
    expect(await boundConn.appliedVersions()).toEqual([1]);
  });
  ...
  it('execution failure output identifies the specific snapshot contract that failed validation', async () => {
    ...
    await expect(up(boundConn, [invalidSnapMigration])).rejects.toThrow(SnapshotMismatchError);
  });
  ...
  it('migration tracking tables update only after snapshot validation and execution succeed', async () => {
    ...
    await expect(up(boundConn, batchMigrations, { targetSnapshot: snapV2 })).rejects.toThrow(SnapshotMismatchError);
