...
The package depends on `@zmdb/sql` and `@zmdb/orm`. `mysql2` is an optional peer and a development dependency. The adapter is typed against the structural `mysql2/promise` connection and pool methods
it actually uses.
