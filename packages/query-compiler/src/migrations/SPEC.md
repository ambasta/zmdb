...
SQLite accepts an inline foreign key whose target table is created later, so `@zmdb/sqlite` permits mutually-referencing table definitions and live tests execute both statements with foreign-key
enforcement enabled. The temporary generic string-dialect compatibility path still carries its older cycle refusal until #675 removes that implementation; it is not a package capability.
...
import type { EmbeddedMigration } from '@zmdb/sqlite/embedded';
...
The canonical package path is now `@zmdb/sqlite/embedded` → `packages/sqlite/src/embedded.ts`, which imports **nothing**. The temporary `@zmdb/query-compiler/migrations/embedded` compatibility leaf
remains until #675, but generated modules no longer reference it. That is the whole discipline, and it has to be structural because there is no bundler here to be hopeful about — Metro has no
dead-code elimination driven by `"sideEffects": false`, so an import that is only unreachable still ships.
