...
describe('Type-Bounded Query Builders in @zmdb/query-compiler', () => {
  const qcSchema = createQueryCompiler<UserSchema>('postgres');
  const qcEntity = createQueryCompiler<UserEntity>('postgres');
  ...
  it('supports inline schema bounding on compiler method calls', () => {
    const untypedQc = createQueryCompiler('postgres');
  ...
  it('type-level verification for column name and value type validation', () => {
    const boundQc = createQueryCompiler<UserEntity>('postgres');
