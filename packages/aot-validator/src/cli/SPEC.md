...
> Part of `@zmdb/aot-validator` (module `src/cli/`). Build-time; the only binary the monorepo publishes (`bin: { "zmdb-codegen": "./src/cli/bin.ts" }`). Package spec: `../../SPEC.md`. Design:
