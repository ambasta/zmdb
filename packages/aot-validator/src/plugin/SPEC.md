...
one front end, and `yarn verify:fixtures` already asserts that the two existing routes agree byte for byte. What this section freezes is the integration, because Metro's shape differs from unplugin's
in three ways that each have a wrong answer that looks right.
...
- `withZmdb(config, { workerCount })` lowers Metro's worker count for a machine that cannot hold N programs. It is not defaulted to 1, because a bundle is mostly files with no call site in them and
  those files want the parallelism.
