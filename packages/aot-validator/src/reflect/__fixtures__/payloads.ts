// The type-driven half of the payload corpus: one derived DTO per variant of each table
// `equivalence.ts` describes.
//
// `payload-types.spec.ts` reflects each of these into a `TypeIR` and compares it against
// `objectTypeFromIR(irFromSchema(value), variant)` — the IR back-end the repository
// validates through. Same claim as `documents.ts` makes for JSON Schema, one layer down:
// the type and the schema value have to describe the same payload, or the repository and
// the emitted validator disagree about the same `create` call.
//
