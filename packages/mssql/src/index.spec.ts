...
import { trustedTable, createQueryCompiler, inc, proposed, UnsupportedFeatureError } from '@zmdb/sql';
...
describe('@zmdb/mssql dialect contract (#672)', () => {
  ...
  it('refuses pagination without ORDER BY', () => {
    expect(() => createQueryCompiler(mssql).selectFrom(trustedTable('users')).limit(10).compile()).toThrow(
      new UnsupportedFeatureError(
  ...
  it('quotes every qualified-name segment and uses p1 through pn placeholders', () => {
    expect(
      createQueryCompiler(mssql)
        .selectFrom(trustedTable('audit]schema.user]events'))
        .where('tenant]id', '=', 7)
