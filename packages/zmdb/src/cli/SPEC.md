...
is a module an application bundle imports, and the format belongs to `@zmdb/query-compiler` — see §4.1.
...
`zmdb`'s other twelve verbs are about a database schema. In that company `graph` reads as the table graph — the foreign-key graph a `pull` walks, the dependency order a `push` sorts DDL into, the
thing a schema visualiser would draw.
