...
**The MySQL family has no transactional DDL**, so that guarantee is available
on the Postgres family, SQLite and SQL Server. On MySQL/SingleStore an
interrupted migration leaves the ledger honest — the row is not written — and
the schema half-applied, which is worse than the reverse and cannot be fixed
from here. The failure message therefore lists the statements that already
ran, because that list is the only way to hand-finish the migration.
