...
Epic #201. Wires the `@zmdb/schema/dto` layer into `BaseRepository<T>` so reads/writes are **typed at the call site** from the declared table `T` — not `Record<string, unknown>`. There are still no
proxies or identity map; SQL stays parameterized and writes validate before execution.
