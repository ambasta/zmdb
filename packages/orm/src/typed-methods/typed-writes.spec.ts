...
import { BaseRepository, type Driver, ValidationError } from '@zmdb/orm';
import { type CreateDTO, type UpdateDTO } from '@zmdb/schema';
...
describe('typed create/update (#206)', () => {
  ...
  describe('strict payload deserialization failure cases', () => {
    it('rejects unknown keys with specific error path and message', async () => {
      const execute = vi.fn(async () => []);
      const repo = new Users({ dialect: officialDialects.postgres, execute } as Driver);
