...
- SQLite is exposed by `@zmdb/sqlite` and its `@zmdb/sqlite/node` subpath. `@zmdb/orm/drivers/sqlite` has been removed.
- SQL Server is exposed by `@zmdb/mssql`; `@zmdb/orm/drivers/mssql` has been removed.
...
`@zmdb/orm` owns only these vendor-neutral interfaces:
...
| `transactional.ts` interface          | `@zmdb/orm`                                                  | no client                                                                             |
