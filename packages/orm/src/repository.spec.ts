...
async function routineCompiler(dialect: 'postgres' | 'mysql' | 'sqlite' | 'cockroach'): Promise<FrozenRoutineCompiler> {
  const loaded: unknown = await import('@zmdb/sql');
  if (typeof loaded !== 'object' || loaded === null) {
    throw new Error('@zmdb/sql did not load as a module record');
  }
  const create: unknown = Reflect.get(loaded, 'createQueryCompiler');
  if (typeof create !== 'function') {
    throw new Error('@zmdb/sql exports no createQueryCompiler');
  }
...
async function callRoutine(
  ...
  if (!isRepositoryCall(call)) {
    throw new Error('@zmdb/orm BaseRepository has no protected call(def, args)');
  }
