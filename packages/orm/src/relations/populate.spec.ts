...
import { RELATION_KINDS, type ColumnIR, type SchemaIR } from '@zmdb/schema/ir';
import { resolveRelation } from '@zmdb/schema/relations';
import { sqlite } from '@zmdb/sqlite';
import { describe, it, expect } from 'vitest';

import { PostSchema, UserSchema } from '../../../schema/src/relations/fixtures.js';
...
describe('many-to-many foreign key name inference', () => {
  it('converts table names to singular camelCase foreign key defaults via resolveRelation', () => {
    const makeIr = (table: string, target: string): SchemaIR =>
      ({
        table,
        primaryKey: ['id'],
        relations: [{ name: 'rel', relation: 'manyToMany', target, via: 'pivot' }],
      }) as unknown as SchemaIR;
    expect(resolveRelation(makeIr('posts', 'tags'), 'rel')).toMatchObject({ baseFk: 'postId', targetFk: 'tagId' });
    expect(resolveRelation(makeIr('users', 'user_addresses'), 'rel')).toMatchObject({
      baseFk: 'userId',
      targetFk: 'userAddressId',
    });
    expect(resolveRelation(makeIr('categories', 'public.users'), 'rel')).toMatchObject({
      baseFk: 'categoryId',
      targetFk: 'userId',
    });
  });
