...
The fixture root deliberately has no `tsconfig.json`. Until the remaining #658–#661 packages exist, the aggregate consumer assertion is an `it.fails`; the ordinary monorepo typecheck must not turn
those four missing imports into an uncontrolled failure outside that assertion.
