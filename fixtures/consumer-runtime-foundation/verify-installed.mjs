...
const sha = async (bytes, algorithm = 'SHA-256', encoding = 'hex') => {
  ...
  if (encoding === 'hex') return typeof digest.toHex === 'function' ? digest.toHex() : Buffer.from(digest).toString('hex');
  return typeof digest.toBase64 === 'function' ? digest.toBase64() : Buffer.from(digest).toString('base64');
};
