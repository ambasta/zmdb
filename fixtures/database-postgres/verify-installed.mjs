...
function main() {
  if (process.env.ZMDB_POSTGRES_URL === undefined) {
  ...
  if (built.status !== 0) throw new Error('yarn build failed before packed PostgreSQL verification');

  const packages = workspacePackages();
  const names = closure(packages, '@zmdb/postgres');
  const scratch = mkdtempSync(join(tmpdir(), 'zmdb-database-postgres-'));
  try {
    ...
    if (installed.status !== 0) throw new Error(`packed PostgreSQL install failed: ${output(installed)}`);

    const tsc = join(ROOT, 'node_modules', '.bin', 'tsc');
    const types = run(tsc, ['--noEmit', '-p', join(app, 'tsconfig.json')], { cwd: app });
    if (types.status !== 0) throw new Error(`packed PostgreSQL declarations failed: ${output(types)}`);

    const runtime = run(process.execPath, [join(app, 'src', 'runtime.mjs')], {
      cwd: app,
      env: { ...process.env, ZMDB_POSTGRES_URL: process.env.ZMDB_POSTGRES_URL },
    });
    if (runtime.status !== 0) throw new Error(`packed PostgreSQL runtime failed: ${output(runtime)}`);
    process.stdout.write(runtime.stdout);
    process.stdout.write(
      `packed PostgreSQL consumer passed with ${names.length} local package archives and consumer-selected pg\n`,
    );
  } finally {
    if (process.env.ZMDB_KEEP_DATABASE_POSTGRES_FIXTURE === undefined) {
