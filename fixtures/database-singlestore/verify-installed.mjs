...
const FIXTURE = join(ROOT, 'fixtures', 'database-singlestore');
...
function main() {
  ...
  if (built.status !== 0) throw new Error('yarn build failed before packed SingleStore verification');

  const packages = workspacePackages();
  const names = closure(packages, '@zmdb/singlestore');
  const scratch = mkdtempSync(join(tmpdir(), 'zmdb-database-singlestore-674-'));
  try {
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            mysql2: fixtureManifest.dependencies.mysql2,
          },
        },
        null,
        2,
    ...
    if (installed.status !== 0) throw new Error(`packed SingleStore install failed: ${output(installed)}`);

    const tsc = join(ROOT, 'node_modules', '.bin', 'tsc');
    const types = run(tsc, ['--noEmit', '-p', join(app, 'tsconfig.json')], { cwd: app });
    if (types.status !== 0) throw new Error(`packed SingleStore declarations failed: ${output(types)}`);

    const runtime = run(process.execPath, [join(app, 'src', 'runtime.mjs')], {
      cwd: app,
      env: { ...process.env, ZMDB_SINGLESTORE_URL: process.env.ZMDB_SINGLESTORE_URL },
    });
    if (runtime.status !== 0) throw new Error(`packed SingleStore runtime failed: ${output(runtime)}`);
    process.stdout.write(runtime.stdout);
    process.stdout.write(
      `packed SingleStore consumer passed with ${names.length} local package archives and consumer-selected mysql2\n`,
    );
  } finally {
    if (process.env.ZMDB_KEEP_DATABASE_SINGLESTORE_FIXTURE === undefined) {
