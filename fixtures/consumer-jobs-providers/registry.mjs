...
export async function startRegistry(packages) {
  ...
  for (const entry of packages) {
    ...
    tarballs.set(entry.manifest.name, {
      ...
      integrity: `sha512-${globalThis.btoa(Array.from(new Uint8Array(await globalThis.crypto.subtle.digest('SHA-512', bytes)), b => String.fromCharCode(b)).join(''))}`,
      shasum: Array.from(new Uint8Array(await globalThis.crypto.subtle.digest('SHA-1', bytes)), b =>
