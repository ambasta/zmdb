...
yarn zmdb codegen --project tsconfig.json --watch
```
...
The compiler package owns the same `compileProject` and `writeCompileResult` APIs for custom tools. Install `@zmdb/compiler@1.0.0-beta.2` with `typescript@^7.0.2` when only the library is needed.
