...
ArkType's achievement is parsing its syntax _in the type system_, so `'number'` becomes `number` with no build step. The cost is that the syntax is ArkType's, not TypeScript's — it covers a large
subset, but a type you already have has to be re-expressed as an ArkType definition to be validated.
...
ArkType's constraint syntax (`'number > 5'`, `'string < 100'`) is more expressive inline than zmdb's rule objects. Where you need arbitrary predicates, `refine` takes a function, which is strictly
more general and less pretty.
...
If a build step is unacceptable and you want type-first validation, ArkType is the better tool — it is the only library in this list that gets close to type-as-schema with no transformer. That is a
real engineering trade, not a consolation.
