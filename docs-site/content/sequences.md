...
`HasDefault` makes `order_number` optional in `CreateDTO<Order>`, which is the part the
compiler needs to know. Which sequence, and the `nextval(...)` call itself, goes in the
...
> [!IMPORTANT]
> This is raw dialect SQL and it belongs next to the `CREATE SEQUENCE`, in the same migration
> — the ordering matters, and a declaration that named the sequence could not enforce it
> anyway. Note this is also _not_ `Serial`: `Serial` means "the database generates it, do not
> send one", which is the same optionality plus an exclusion from `CreateDTO` and a `SERIAL`
> in the generated DDL. Use `HasDefault` when you own the sequence.
