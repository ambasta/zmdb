...
Increment and toggle use the same closed expression vocabulary in `UpdateBuilder.set()` and `BaseRepository` patches. Updating different rows to different values is a separate, wider `CASE` / `VALUES`
problem; `updateMany` covers one validated patch over all matching rows.
...
| [Case-insensitive unique](./guide-case-insensitive-unique.html)               | Expression index on the PostgreSQL family/SQLite; generated column on the MySQL family/SQL Server |
...
- **`References` is a tag, not a function.** `authorId: number & Sql<'integer'> & References<'users.id'>`. The target is a `table.column` string literal, so nothing has to be imported — and nothing
