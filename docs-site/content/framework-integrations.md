...
| React Native | optional | @zmdb/react-native | react<br>react-native      | [client-react-native](./client-react-native.html) | `packages/compiler/src/metro/metro.spec.ts`<br>`packages/react-native/src/index.spec.ts`<br>`packages/react-native/src/metro.spec.ts`<br>`packages/react-native/src/packed-consumer.spec.ts`<br>`fixtures/client-adapters`<br>`fixtures/consumer-metro` |
