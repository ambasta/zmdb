...
`@zmdb/app/observability` and the HTTP instrumentation have no OpenTelemetry dependency. To adapt application-owned OpenTelemetry objects, install `@zmdb/otel@1.0.0-beta.1 @opentelemetry/api@^1.9.1`.
Neither is part of the `zmdb` default install. The adapter owns no provider, processor, exporter, collector client, global registration, or shutdown hook.
