...
The driver returns JSON columns as parsed values in the Postgres family
(`json`/`jsonb`) and as **strings** in the MySQL family, SQLite and SQL Server.
That difference is the driver's, not zmdb's:
