...
The six dialect values all have complete golden-SQL matrix entries, including explicit refusals. SQLite runs in-process. SQL Server has an opt-in real-server suite through `ZMDB_MSSQL_URL`.
CockroachDB and SingleStore currently have no live-server suite in this repository, so their pages separate supported compiler behavior from deployment qualification.
