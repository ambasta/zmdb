...
Install the adapter and its sole external peer with `yarn add @zmdb/otel@1.0.0-beta.1 @opentelemetry/api@^1.9.1`. Neither is part of the `zmdb` default install. The application selects and owns any
SDK, provider, processor, exporter, sampler, collector connection, global registration, flush, and shutdown.
