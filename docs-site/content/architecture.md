...
The Vercel AI SDK manifest and clean packed-consumer proof now use the frozen supported floor, `7.0.93`. The target release classification covers every public row above exactly once: eight cohesive
core packages, 27 independently versioned integrations, and one independently versioned tooling package.
...
The Vercel adapter's clean packed consumer builds and installs its release tarballs with exact AI SDK `7.0.93`, typechecks representative public usage with the documented `skipLibCheck: true`, and
executes a streamed tool call through the real SDK. Both imports resolve from the temporary consumer's `node_modules`, without workspace links or TypeScript path aliases.
