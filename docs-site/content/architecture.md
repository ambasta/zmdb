...
The generated `ai@^7.0.83` row is observed metadata from the current manifest, not the frozen support promise. The supported and tested Vercel AI SDK floor is `7.0.93`; changing that manifest and
removing its lower-bound alias belongs to issue #748. The target release classification covers every public row above exactly once: eight cohesive core packages, 27 independently versioned
integrations, and one independently versioned tooling package.
...
5. Classify tooling exports and each optional peer by exact export/bin selector. Required peers remain confined to technology-selected integration packages.
...
The commands below describe the checked-in pre-#746 tooling. They still require one reviewed root changelog and one version for all 36 public packages:
...
Do not use those commands to approximate a partial release. The frozen target keeps eight core packages lockstep, gives each of the 27 integrations and `@zmdb/migrations` an independent version, uses
...
The Vercel adapter's supported and tested floor is AI SDK `7.0.93`. The #746 packed-consumer probe built and packed `@zmdb/query-compiler`, `@zmdb/schema-core`, `@zmdb/ai`, and `@zmdb/ai-vercel`,
installed those tarballs with exact `ai@7.0.93` outside the repository, typechecked representative public usage with the documented `skipLibCheck: true`, resolved both packages from the temporary
consumer's `node_modules`, and executed the real `description`, `execute`, and `inputSchema` fields. The current manifest's older range and alias remain implementation state for issue #748, not a
support guarantee.
