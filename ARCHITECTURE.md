...
7. **Zero third-party dependencies on the query/validation hot path.** Runtime execution depends only on `@zmdb/*` packages and Node built-ins. Third-party integrations (a `pg` driver, a Hono adapter)
   are _optional_ and structurally typed. The tooling exception is `oxfmt`, pinned by `@zmdb/migrations` for declaration emission; the query compiler and root, runner, and embedded migration entries
   do not reach it.
...
>
> Dated assertion measurements are preserved in [ADR 0001](./docs/adr/0001-architecture-and-native-graph-history.md). The current checker reports the present source findings.
>
...
**Subpath exports are the default; a new package is the exception.** We would rather ship `@zmdb/schema/dto` than a premature `@zmdb/dto`.
...
complete database vertical through an optional identity subpath without installing its client.
...
to an optional peer and development dependency. `@zmdb/jobs-postgres` depends on it rather than the generic repository, and the umbrella exposes it through the optional `zmdb/postgres` identity
facade.
...
`@zmdb/compiler` is a tooling leaf over AI, the validator runtime, query-compiler, and schema-core; only explicit tooling facades and compiler hosts reach it.
...
- **SvelteKit depends on client and Svelte, never server packages.** Its browser export reuses the Svelte stores; its server export owns request-local `event.fetch`, explicit credential forwarding,
  load/error identity, and request cancellation.
- **aot-validator is the compiler-free runtime.** It depends only on schema-core and owns validation, serialization, errors, and emitted-code helpers.
- **compiler owns every TypeScript front-end concern.** Reflection, `TypeIR` production, emission, transforms, project compilation, unplugin/Metro adapters, lint rules, testing utilities, and
  filesystem-backed config loading live in `@zmdb/compiler`; runtime roots cannot reach it.
- **repository is the composition layer** — it wires schema + query compilation + validation into CRUD and owns only vendor-neutral driver protocols.
...
- **RabbitMQ depends only on app.** It implements the public transport strategy with bounded prefetch, confirmed delayed retries, and owned dead-letter topology while owning only the required
  `amqplib` peer.
- **Redis depends only on app.** It implements concrete Pub/Sub channels and process-owned request/reply subscriptions while owning only the required `redis` peer.
...
Use the [generated package reference](./docs-site/content/package-reference.md) for roles, installs and versions, and the generated policy graph in §3.10 for dependency and reachability rows. These
projections use the canonical catalog and manifests. The former handwritten inventory is preserved in [ADR 0001](./docs/adr/0001-architecture-and-native-graph-history.md).
...
six database verticals are now package-owned. Generic compiler, schema, and repository code accepts only the injected protocol object. The umbrella exposes the six explicit optional identity subpaths
`zmdb/sqlite`, `zmdb/postgres`, `zmdb/mysql`, `zmdb/mssql`, `zmdb/cockroach`, and `zmdb/singlestore`.
...
| -------------------------------------------------------------- | -------------------------------------------------------------- | --------------------------------------------------------------------- |
| `@zmdb/sql`                                                    | `@zmdb/orm`                                                    | required generic edge                                                 |
| `@zmdb/migrations`, `@zmdb/sql`, `@zmdb/orm`                   | `@zmdb/sqlite`, `@zmdb/postgres`, `@zmdb/mysql`, `@zmdb/mssql` | required vertical edges                                               |
| `@zmdb/migrations`, `@zmdb/sql`, `@zmdb/orm`, `@zmdb/postgres` | `@zmdb/cockroach`                                              | required child edge                                                   |
| `@zmdb/migrations`, `@zmdb/sql`, `@zmdb/orm`, `@zmdb/mysql`    | `@zmdb/singlestore`                                            | required child edge                                                   |
...
optional peer selected by the application. The exact protocol types live below the database packages in `@zmdb/sql` and `@zmdb/orm`, so the graph has no reverse edge.
...
credential, TypeIR walker, or workspace path. UI adapters remain the separate client-framework layer frozen by #688.
...
Arrows point from consumer to direct dependency. `@zmdb/schema` has no reverse AI edge, no LLM export and no provider peer in the final graph. Integration SDKs are optional peers of only their own
opt-in packages; the Vercel adapter receives the branded factory and never imports the SDK. Installing the provider-neutral packages does not install or resolve an SDK. `@zmdb/mcp` uses platform APIs
and depends only on `@zmdb/ai`.

`toolFor<T>()` remains an AOT callee; its declared source and generated witness imports are `@zmdb/ai`, and `@zmdb/compiler` consumes the shared document producer through `@zmdb/ai/compiler`.
Generated OpenAPI-tool modules import `OpenApiGeneratedTool` from `@zmdb/ai/http`.
...
`@zmdb/compiler` keeps the only reflection session, protobuf/service-IR walk and emitter. `@zmdb/protobuf` owns the five source calls, service-artifact types and generated wire runtime, but no
...
The catalog deliberately does not own versions, dependency ranges, changelogs, npm tags, publication credentials, publish order, compatibility timing, or partial-release behavior. Architecture policy
owns dependency direction and therefore publish order. Release groups, version movement, and compatibility floors are owned by [`scripts/release/policy.mjs`](./scripts/release/policy.mjs) under the
contract in [`scripts/release/SPEC.md`](./scripts/release/SPEC.md); release tooling combines those authorities through the governance snapshot.

The historical `44d8fa4a` measurement of 74 root symbols and 13 export-map entries, plus the target taxonomy and eager-import rules, are frozen in [`packages/zmdb/SPEC.md`](./packages/zmdb/SPEC.md).
The current #651 facade measures 118 catalog-owned root names (68 runtime and 50 type-only) and 48 manifest entries (the root plus 47 named subpaths). Configuration ownership is frozen in
[`packages/zmdb/src/config/SPEC.md`](./packages/zmdb/src/config/SPEC.md), and the current package inventory plus required catalog consumers and rejection rules are frozen in
...
### 3.10 Canonical architecture and release policy (#722–#728, #746)

The canonical architecture policy, read-only graph model, workspace-edge verifier, runtime-reachability verifier, package-metadata verifier, and original lockstep release verifier now exist. Product
membership stays owned by [`scripts/product/catalog.mjs`](./scripts/product/catalog.mjs). [`scripts/architecture/policy.mjs`](./scripts/architecture/policy.mjs) attaches exactly one
...
The [governance snapshot](./scripts/architecture/governance.mjs) composes catalog membership, architecture policy, manifests, reachability, metadata, release policy and owned exceptions. Focused
commands and the aggregate query consume those same records. [Native GitHub relationships](./scripts/roadmap/native-relationships.mjs) determine issue ownership and actionability. Current contributor
operations are described in [CONTRIBUTING.md](./CONTRIBUTING.md); dated inventories and the completed native cutover are preserved in
...
The generated reachability row for `@zmdb/ai-vercel` reports the implemented `ai@^7.0.93` peer. Exact `7.0.93` is the supported floor, the development fixture, and the version installed with the
published zmdb tarballs by the package-owned external-consumer proof.
...
Package admission and dependency changes follow the [single contributor workflow](./CONTRIBUTING.md#package-and-dependency-changes). Catalog, manifest, architecture and release declarations change
together; generated views are regenerated from those sources. Use focused issue checks and one applicable integrated gate after composition.
...
architecture fixtures and is run by `yarn verify:architecture-zones` in CI. [`verify-package-metadata.mjs`](./.github/scripts/verify-package-metadata.mjs) verifies core version identity, independent
version ownership, same-core and explicit cross-unit ranges, export/bin targets, repository directories, and release-policy peer projections before root builds or publication.
[`verify-runtime-reachability.mjs`](./.github/scripts/verify-runtime-reachability.mjs) starts independently from every export and executable: ordinary runtime entries cannot reach compiler/build
tools, REPL/devtools modules or an optional peer assigned to another entry. Tooling exports and optional peers are explicit per-package policy exceptions, never inferred from a directory name, and
stale exceptions fail. `verify:exports` delegates reachability to that verifier. Relative source imports retain NodeNext `.js` specifiers, and `allowImportingTsExtensions` remains `false`.

The current release groups, compatibility ranges, changelog and tag rules come from [release policy](./scripts/release/policy.mjs) and its [SPEC](./scripts/release/SPEC.md). Follow
[PUBLISHING.md](./PUBLISHING.md) for release operations.
...
[`scripts/architecture/SPEC.md`](./scripts/architecture/SPEC.md). The executable release and compatibility contract is in [`scripts/release/SPEC.md`](./scripts/release/SPEC.md), with the operator
workflow in [`PUBLISHING.md`](./PUBLISHING.md).
...
Migration/introspection logic still lives in `@zmdb/sql`, and the executable implementation still lives in `zmdb` until their separate extraction issues land.
...
The implemented and target packages are independently callable:

- `@zmdb/compiler` now owns the one TypeScript front end, `TypeIR` production, AOT emitters, transforms, unplugin/Metro adapters, lint integration, compiler testing utilities and the canonical
...
not duplicated implementations. The root does not eagerly import them. The old `zmdb/unplugin` spelling and implementation are deleted; the configured plugin is owned by the compiler root. Metro
remains available only through its explicitly selected `@zmdb/compiler/metro` adapter entry.
...
[`verify-tooling-ownership.SPEC.md`](./.github/scripts/verify-tooling-ownership.SPEC.md), which assigns every shipped/build-input path, workspace dependency edge, remaining tooling binary, and
checked-in generated artifact exactly once.
...
[`packages/cli/SPEC.md`](./packages/cli/SPEC.md). The compiler manifest, product-catalog row and architecture-policy row are admitted; the remaining roadmap packages stay under §3.10 until their
separate issues admit those three surfaces atomically. Release membership and deterministic publish order come only from the catalog and policy DAG, never from a second tooling-package list.
...
| `@zmdb/web`           | app, schema-core                                                             | compiler, TypeScript (optional tooling peers) |
| `@zmdb/jobs`          | app                                                                          | none                                          |
...
The cutover deletes `@zmdb/schema`, `@zmdb/sql`, `@zmdb/validator`, and `@zmdb/orm` and all of their old subpaths. They do not survive as forwarding packages. The remaining file and export ownership
map is normative in [`.github/scripts/verify-runtime-foundation.SPEC.md`](./.github/scripts/verify-runtime-foundation.SPEC.md).

### 3.14 Default runtime and selected jobs (#753)

The default product is the packed `zmdb` schema/ORM/validation/application/web journey. Database providers are selected explicitly. Jobs is a selected first-party capability: it shares product
...
Portable jobs has one direct edge to `@zmdb/app` and a six-package workspace closure including itself. Each provider adds exactly itself and its database/migration closure. `zmdb` has no runtime
`jobs` export and no static, optional, dynamic, or private-source route to jobs.
...
The [ADR index](./docs/adr/index.md) preserves superseded measurements, diagrams and migration decisions. Current invariants remain in this document and their owning SPECs; historical text does not
authorise a current dependency, alias, exception or release action.
