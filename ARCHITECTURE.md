...
adapter; SQL Server owns the structural node-mssql adapter and declares `mssql` as an optional client peer. During the transition, `@zmdb/jobs` depends on SQLite for the in-memory queue backend and
`zmdb` retains identity facade subpaths. #675 owns the final product/config/CLI cutover and optional facade shape.
...
- **repository is the composition layer** — it wires schema + compiler + validator into CRUD and owns only vendor-neutral driver protocols.
...
| `@zmdb/mssql`              | Complete SQL Server traits, DDL/migrations/refusals, catalog introspection, capabilities, and structural node-mssql driver                            | query-compiler, repository; `mssql` (optional peer)                                                       |
...
Measured from `scripts/product/catalog.mjs`, `scripts/architecture/policy.mjs`, and the admitted manifests: **37 catalog packages**, **74 direct workspace edges**, and canonical rings **0–7**.
...
| 7    | facade      | `zmdb`                     | `@zmdb/app`<br>`@zmdb/aot-validator`<br>`@zmdb/jobs`<br>`@zmdb/migrations`<br>`@zmdb/mssql`<br>`@zmdb/postgres`<br>`@zmdb/query-compiler`<br>`@zmdb/repository`<br>`@zmdb/schema-core`<br>`@zmdb/sqlite`<br>`@zmdb/web` |
