...
> and OpenAPI-derived tools** harness. **1,971 tests green** across 206 files,
> including real `node:sqlite` E2E, a Kysely
> head-to-head, and the full validation + ORM benchmark suites (real PostgreSQL).
> Alongside them, **316 expected-failing tests** hold the frozen specs of features
...
> MikroORM, NestJS and Typia run between them: 496 are answered by a named zmdb
> test, 246 are argued against in writing, and `yarn verify:api-coverage` fails
> on a suite that is neither.
> Of the 276 docs-site pages, 214 document a capability that exists, 50 are marked
