...
Twelve published packages. The data, application, and HTTP stack remains one install (`zmdb`); provider-neutral AI tooling is exposed from `@zmdb/ai` without a provider or framework SDK dependency,
while Anthropic and LangChain integrations ship separately as `@zmdb/ai-anthropic` and `@zmdb/ai-langchain`.
...
`@zmdb/client` and `@zmdb/protobuf` are independent dependency-free roots outside this spine. Neither opt-in AI integration is re-exported by `zmdb`.
...
| **RISK-5**  | **Breadth vs. maintainability** — twelve packages spanning the cohesive product, zero-dependency runtimes, provider-neutral AI tooling, and two opt-in integrations, maintained by a small team.                                                                                                                                                                                                                                                      | Splitting doctrine (`ARCHITECTURE.md` §3.1): subpath exports are the default, a new package is the exception, and dissolving a package is an encouraged refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Twelve published packages in one acyclic DAG, with the schema as the shared source of truth for the SQL, HTTP, and provider-neutral AI boundaries and opt-in Anthropic/LangChain integrations at the edge (§7 — the seam).               |
