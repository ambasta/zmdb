...
| **REQ-TF-4**  | The whole DTO suite derives from the tagged type: `Entity`, `CreateDTO`, `UpdateDTO`, `WhereDTO`, `ReadDTO`, `PrimaryKey`, order-by, pagination, projection, and populated/join shapes.                                                                              | The existing REQ-SC-2…REQ-SC-5 type tests pass unchanged with the tagged interface substituted for `typeof Schema`.                       |
...
| **REQ-TF-12** | `defineSchema` keeps working throughout the migration; adoption is incremental and per-table.                                                                                                                                                                        | Both declaration styles pass the same repository and web E2E suites in one project.                                                       |
