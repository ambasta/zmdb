...
Thirty-six published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
`@zmdb/mssql` owns the complete SQL Server vertical, and `@zmdb/cockroach` extends PostgreSQL one-way. Provider-neutral AI tooling is exposed from `@zmdb/ai` without a provider or framework SDK
dependency, Anthropic, LangChain, and Vercel integrations ship as separate opt-in packages, transport-neutral MCP client/server cores ship from `@zmdb/mcp` without an MCP SDK dependency, and
`@zmdb/otel` adapts caller-owned OpenTelemetry API objects without placing that peer on the cohesive product. Typed gRPC, core NATS, RabbitMQ, and Redis Pub/Sub each ship as an independently selected
transport package with their sole required client peer. Generated-client lifecycle integrations for React, React Native, Angular, Vue, Svelte, Solid, Next.js, Nuxt, and SvelteKit are independently
selected and do not place their framework peers on the cohesive product.
...
@zmdb/ai               (provider-neutral tools; depends directly on schema-core)
        ...
        ▼
@zmdb/aot-validator    (+ direct schema-core dependency)
        ▼
...
| `@zmdb/aot-validator`      | **Typia**                              | AOT transformer + `is`/`assert`/`validate`/`equals`/`random`, tags, unions, transforms, JSON Ser/De                                                                        | ai, schema-core                                                                                           |
...
| `zmdb`                     | the data/app/web stack                 | Umbrella meta-package (curated root + subpath re-exports; no jobs, jobs provider, AI, MCP, OTel, UI, or transport public re-export)                                        | app, aot-validator, schema-core, query-compiler, repository, sqlite, web; mssql/postgres (optional peers) |
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Thirty-six published packages in one acyclic DAG, with the schema as the shared source of truth for SQL, database verticals, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, Redis, and UI-adapter boundaries (§7 — the seam). |
