...
Twenty-two published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
`@zmdb/client` and `@zmdb/protobuf` are independent dependency-free roots outside this spine. None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, or Redis integration packages is
re-exported by `zmdb`.
...
| `@zmdb/react`              | React generated-client integration     | Context, query, and mutation lifecycle bindings over generated clients, with caller-owned client and component lifecycles                                                   | client; `react` (required peer)                                          |
...
Counted off a real parse tree, per package, over the **243 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
...
| `@zmdb/protobuf`       |          0 |              0 | —                                                                                    |
| `@zmdb/repository`     |          6 |              7 | driver row → `Entity<S>`, the subclass statics, cursor payload                       |
| `@zmdb/query-compiler` |          1 |              1 | the `compile()` duck-type guard                                                      |
| `@zmdb/app`            |          3 |              7 | metadata slots, DI token → instance, and state branding                              |
| `@zmdb/web`            |          5 |             13 | HTTP decorator metadata and request/transport boundary reads                         |
| `zmdb` (umbrella)      |          0 |              0 | —                                                                                    |
