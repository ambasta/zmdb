...
Sixteen published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
that peer on the cohesive product.
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Sixteen published packages in one acyclic DAG, with the schema as the shared source of truth for SQL, database verticals, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, MCP, and OpenTelemetry boundaries (§7 — the seam). |
