...
Thirty-five published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
repository and alone declares the optional SQL Server client peer.
...
| `@zmdb/mssql`              | SQL Server implementation vertical     | T-SQL traits, migrations, exact catalog introspection, capability/refusal metadata, and a structural node-mssql driver                                                      | query-compiler, repository; `mssql` (optional peer)                                                       |
| `@zmdb/postgres`           | PostgreSQL implementation vertical     | PostgreSQL SQL, migrations, catalog introspection, structural pg driver, streaming, cancellation, and family extension points                                               | query-compiler, repository; `pg` (optional peer)                                                          |
| `@zmdb/sqlite`             | SQLite implementation vertical         | SQLite SQL, migrations, introspection, embedded migration runner, declarations, and structural `node:sqlite` driver                                                         | query-compiler, repository                                                                                |
| `@zmdb/mysql`              | MySQL implementation vertical          | MySQL SQL/refusals, DDL/migrations, catalog introspection, structural mysql2 driver, capabilities, and mandatory packed live acceptance                                     | query-compiler, repository; `mysql2` (optional peer)                                                      |
...
| `@zmdb/jobs`               | NestJS queues and scheduling           | Typed queues, workers, dead letters, scheduling, leases, and the built-in SQLite memory backend                                                                             | app, query-compiler, repository, sqlite                                                                   |
| `@zmdb/jobs-postgres`      | PostgreSQL job storage                 | PostgreSQL `JobStore` adaptation over caller-owned pools and clients                                                                                                        | jobs, postgres; `pg` (required peer)                                                                      |
...
The NestJS replacement remains one product, but its dependency firebreaks become three packages:
...
| `@zmdb/jobs` | queues, workers, dead letters, scheduling, leases and the built-in SQLite memory backend                                                       |

All three are reached cohesively through `zmdb`, `zmdb/app`, `zmdb/web` and `zmdb/jobs`. The split moves ownership; it does not introduce a second product, duplicate implementation, compatibility
forwarder or per-request abstraction layer.

The core package graph is acyclic: `web -> app`, `jobs -> app`, and neither app nor either sibling imports back. App/web/jobs declare only workspace runtime dependencies and Node built-ins and have no
third-party runtime peer. gRPC, brokers, PostgreSQL jobs and OpenTelemetry are separately installed adapters that depend inward on app or jobs.
...
| **REQ-SV-5** | `@zmdb/jobs` owns queues/scheduling and the SQLite memory backend; PostgreSQL lives only in `@zmdb/jobs-postgres`.                                              | Manifest/peer oracle and packed memory-backend consumer.                                                                                                              |
...
| **REQ-SV-8** | `zmdb`, `zmdb/app`, `zmdb/web` and `zmdb/jobs` are explicit curated re-exports with collision checks and runtime identity equal to their direct-package source. | Packed consumer asserts imports/typechecks and `===` identity outside the workspace.                                                                                  |
