...
outcome. The two that are not are marked ⚠️ and say what is left: REQ-TF-4's second
...
| **REQ-TF-4**  | The whole DTO suite derives from the tagged type: `Entity`, `CreateDTO`, `UpdateDTO`, `WhereDTO`, `ReadDTO`, `PrimaryKeyOf`, order-by, pagination, projection, and populated/join shapes. Each takes the tagged type only — no dispatch on a schema value.                                                                                       | ⚠️ Met for the derivations, not yet for the dispatch. `derive/type-derivation-tagged.type-test.ts` is `type-derivation.type-test.ts` with `S` rebound to the tagged interface — identical key sets and optionality, every derivation mutually assignable with its schema-value twin — and `derive/tagged-dto.type-test.ts` covers the suite directly. Not literally _unchanged_; that contradicted REQ-TF-5, see the note below. **The second sentence is outstanding:** the root `Entity`/`CreateDTO`/`UpdateDTO` still branch on `S extends TaggedSchema<infer T>` because `./dto` is parameterised on the schema value. Collapsing that branch is `schema-core/SPEC.md` §4. |
