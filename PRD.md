...
Fifteen published packages. The data, application, and HTTP stack remains one install (`zmdb`); provider-neutral AI tooling is exposed from `@zmdb/ai` without a provider or framework SDK dependency,
Anthropic, LangChain, and Vercel integrations ship as separate opt-in packages, transport-neutral MCP client/server cores ship from `@zmdb/mcp` without an MCP SDK dependency, and `@zmdb/otel` adapts
caller-owned OpenTelemetry API objects without placing that peer on the cohesive product.
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Fifteen published packages in one acyclic DAG, with the schema as the shared source of truth for the SQL, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, MCP, and OpenTelemetry boundaries (§7 — the seam).  |
