...
never run. All four are covered now: `Fts<'users_fts'>` went into the equivalence corpus,
where `defineSchema`'s `ftsTable` option proves both front-ends agree about it, and the
boolean spelling plus the two missing cardinalities went in beside it as assertions of their
own. A vocabulary entry nothing has ever declared is an entry whose behaviour is a guess, so
