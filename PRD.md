...
> `benchmarks/RESULTS.md` (what is measured), `COOKBOOK.md` (how it is used). **Last revised:** 2026-09-05.
...
Twenty-nine published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
lifecycle integrations for React, Angular, Vue, Svelte, Solid, Next.js, and Nuxt are independently selected and do not place their framework peers on the cohesive product.
...
and Vue adapter. Each alone declares its framework peers. None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, or Redis integration packages is re-exported by `zmdb`.
...
Counted off a real parse tree, per package, over the **268 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
...
| **RISK-5**  | **Breadth vs. maintainability** — twenty-nine packages spanning the cohesive product, complete database verticals, zero-dependency runtimes, provider-neutral AI tooling, selected provider integrations, MCP, gRPC, NATS, RabbitMQ, Redis, and OpenTelemetry adaptation, maintained by a small team.                                                                                                                                                 | Splitting doctrine (`ARCHITECTURE.md` §3.1): subpath exports are the default, a new package is the exception, and dissolving a package is an encouraged refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
