...
Twenty-one published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
that peer on the cohesive product. Typed gRPC, core NATS, and RabbitMQ each ship as an independently selected transport package with their sole required client peer.
...
`@zmdb/client` and `@zmdb/protobuf` are independent dependency-free roots outside this spine. None of the three opt-in AI integrations, `@zmdb/mcp`, or `@zmdb/otel` is re-exported by `zmdb`.
...
| `zmdb`                 | the data/app/web stack                 | Umbrella meta-package (curated root + subpath re-exports; no AI, MCP, or OTel public re-export)                                                                             | app, aot-validator, schema-core, query-compiler, repository, web |
...
| **RISK-5**  | **Breadth vs. maintainability** — nineteen packages spanning the cohesive product, complete database verticals, zero-dependency runtimes, provider-neutral AI tooling, selected provider integrations, MCP, gRPC, and OpenTelemetry adaptation, maintained by a small team.                                                                                                                                                                           | Splitting doctrine (`ARCHITECTURE.md` §3.1): subpath exports are the default, a new package is the exception, and dissolving a package is an encouraged refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Twenty-one published packages in one acyclic DAG, with the schema as the shared source of truth for SQL, database verticals, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, MCP, OpenTelemetry, gRPC, NATS, and RabbitMQ boundaries (§7 — the seam). |
