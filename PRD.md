...
Twenty-eight published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
that peer on the cohesive product. Typed gRPC, core NATS, RabbitMQ, and Redis Pub/Sub each ship as an independently selected transport package with their sole required client peer.
...
`@zmdb/vue`, `@zmdb/svelte`, and `@zmdb/solid` are opt-in leaves over `@zmdb/client`, while `@zmdb/next` depends inward on both the client and React adapter. Each alone declares its framework peers.
...
Counted off a real parse tree, per package, over the **248 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
...
| Metric                                     | 2026-08-31 (grep, 67 files) | 2026-09-02 (parse tree, 84 files) | 2026-09-03 (DSL deleted) | 2026-09-05 current | Ceiling |
| ------------------------------------------ | --------------------------: | --------------------------------: | -----------------------: | -----------------: | ------: |
...
Per-package distribution of the 2026-09-05 recount:
