...
an MCP SDK dependency, and `@zmdb/otel` adapts caller-owned OpenTelemetry API objects without placing that peer on the cohesive product. Typed gRPC, core NATS, RabbitMQ, and Redis Pub/Sub each ship as
an independently selected transport package with their sole required client peer. Generated-client lifecycle integrations for React, React Native, Angular, Vue, Svelte, Solid, Next.js, Nuxt, and
SvelteKit are independently selected and do not place their framework peers on the cohesive product.
...
`@zmdb/vue`, `@zmdb/svelte`, and `@zmdb/solid` are opt-in leaves over `@zmdb/client`, while `@zmdb/react-native` depends inward on the client and React adapter, `@zmdb/next` depends inward on the
client and React adapter, `@zmdb/nuxt` depends inward on the client and Vue adapter, and `@zmdb/sveltekit` depends inward on the client and Svelte adapter. Each alone declares its framework peers.
None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, or Redis integration packages is re-exported by `zmdb`. `@zmdb/mssql` is a selected database vertical over query-compiler and
repository and alone declares the optional SQL Server client peer. Jobs and both jobs providers are likewise selected direct packages rather than `zmdb` dependencies or facade entries.
...
| `@zmdb/angular`            | application-local Angular client glue  | Dependency injection, signals, `DestroyRef` cleanup, Observable cancellation, and request-local generated-client ownership                                                 | `@angular/core`, `rxjs` (required peers)                                                                                        |
| `@zmdb/react`              | application-local React client glue    | Context, query and mutation hooks with dependency-change cancellation, stale-result suppression, and component-tree isolation                                              | client; `react` (required peer)                                                                                                 |
| `@zmdb/react-native`       | application-local native client glue   | React hook reuse plus AppState cancellation, explicit offline dispatch policy, and injected connectivity and credential-store ownership                                    | client, react; `react`, `react-native` (required peers)                                                                         |
...
| `@zmdb/vue`                | application-local Vue client glue      | Application plugin plus typed query and mutation composables with watcher/effect-scope cancellation and per-application SSR isolation                                      | client; `vue` (required peer)                                                                                                   |
| `@zmdb/svelte`             | application-local Svelte client glue   | Typed context, lazy query and mutation stores, final-subscriber cancellation, stale-result suppression, and request-local server ownership                                 | client; `svelte` (required peer)                                                                                                |
...
| `@zmdb/otel`               | OpenTelemetry API integration          | Adapts caller-owned tracers and meters to the app observability ports without owning a provider, SDK, exporter, collector, or ambient context                              | app; `@opentelemetry/api` (required peer)                                                                                       |
...
Both halves are asserted, on the real bundle, in `packages/compiler/src/codegen/consumer-fixtures.spec.ts`. What the AC was reaching for — no interpreter, no descriptor walk, no allocation to answer a
yes/no question — holds, and is the part REQ-AV-2 states directly.
...
| ID          | Risk                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RISK-1**  | ~~**The AOT premise is unearned out of the box.**~~ **Closed 2026-09-02.** The premise needed a route that did not assume a bundler, and `@zmdb/compiler` project compilation is it: it compiles checks to files next to the source, rewrites call sites to import them, and check mode fails a build whose committed output is stale. Both routes are fixtures in CI, emitting the same code (§9.2).                                                 | Keep it closed by keeping the fixtures representative: `yarn verify:fixtures` runs the repository compiler wrapper in check mode, and `packages/compiler/src/codegen/consumer-fixtures.spec.ts` fails if the two fixtures stop being the same program. **Residual risk:** a consumer who wires neither route gets a loud missing-witness failure at runtime, so documentation and the build-path smoke test remain part of the contract.                                                                                                                                                                                                                                                                                               |
