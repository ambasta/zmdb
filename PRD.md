...
Fourteen published packages. The data, application, and HTTP stack remains one install (`zmdb`); provider-neutral AI tooling is exposed from `@zmdb/ai` without a provider or framework SDK dependency,
Anthropic, LangChain, and Vercel integrations ship as separate opt-in packages, and transport-neutral MCP client/server cores ship from `@zmdb/mcp` without an MCP SDK dependency.
...
@zmdb/ai               (provider-neutral tools; depends directly on schema-core)
        ...
        ├────────▶ @zmdb/ai-langchain  (optional LangChain adapter; temporary schema-core edge)
...
`@zmdb/client` and `@zmdb/protobuf` are independent dependency-free roots outside this spine. None of the three opt-in AI integrations or `@zmdb/mcp` is re-exported by `zmdb`.
...
| `@zmdb/schema-core`    | MikroORM entities, Zod/TypeBox schemas | Schema DSL, compile-time type derivation (Entity / Create / Update / read DTOs), relations, custom types, seeding, and OpenAPI. Four old `./llm*` paths remain temporary compatibility implementation during extraction. | query-compiler                                                   |
...
| `@zmdb/ai-langchain`   | LangChain tool integration             | Optional `DynamicStructuredTool` fields over the shared AI tool document and validation runtime; sole owner of the `@langchain/core` peer                                                                                | ai, schema-core temporarily; `@langchain/core` (optional peer)   |
...
| `zmdb`                 | the data/app/web stack                 | Umbrella meta-package (curated root + subpath re-exports; no AI or MCP public re-export)                                                                                                                                 | app, aot-validator, schema-core, query-compiler, repository, web |
...
Counted off a real parse tree, per package, over the **241 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
...
| **RISK-5**  | **Breadth vs. maintainability** — fourteen packages spanning the cohesive product, zero-dependency runtimes, provider-neutral AI tooling, three opt-in integrations, and an MCP protocol core, maintained by a small team.                                                                                                                                                                                                                            | Splitting doctrine (`ARCHITECTURE.md` §3.1): subpath exports are the default, a new package is the exception, and dissolving a package is an encouraged refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Fourteen published packages in one acyclic DAG, with the schema as the shared source of truth for the SQL, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, and MCP boundaries (§7 — the seam).                |
