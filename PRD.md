...
Thirty-two published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
zero-third-party-client SQLite option, while `@zmdb/postgres` owns the complete PostgreSQL vertical behind an optional `pg` peer. Provider-neutral AI tooling is exposed from `@zmdb/ai` without a
provider or framework SDK dependency, Anthropic, LangChain, and Vercel integrations ship as separate opt-in packages, transport-neutral MCP client/server cores ship from `@zmdb/mcp` without an MCP SDK
dependency, and `@zmdb/otel` adapts caller-owned OpenTelemetry API objects without placing that peer on the cohesive product. Typed gRPC, core NATS, RabbitMQ, and Redis Pub/Sub each ship as an
independently selected transport package with their sole required client peer. Generated-client lifecycle integrations for React, React Native, Angular, Vue, Svelte, Solid, Next.js, Nuxt, and
