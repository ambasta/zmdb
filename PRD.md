...
Thirty-three published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
zero-third-party-client SQLite option, `@zmdb/postgres` owns the complete PostgreSQL vertical behind an optional `pg` peer, and `@zmdb/mysql` owns the complete MySQL compiler-to-mysql2 vertical behind
an optional `mysql2` peer. Provider-neutral AI tooling is exposed from `@zmdb/ai` without a provider or framework SDK dependency, Anthropic, LangChain, and Vercel integrations ship as separate opt-in
packages, transport-neutral MCP client/server cores ship from `@zmdb/mcp` without an MCP SDK dependency, and `@zmdb/otel` adapts caller-owned OpenTelemetry API objects without placing that peer on the
cohesive product. Typed gRPC, core NATS, RabbitMQ, and Redis Pub/Sub each ship as an independently selected transport package with their sole required client peer. Generated-client lifecycle
integrations for React, React Native, Angular, Vue, Svelte, Solid, Next.js, Nuxt, and SvelteKit are independently selected and do not place their framework peers on the cohesive product.
...
>
> The 2026-09-06 count covers 275 shipped files: **53 framework assertions with 54 `// boundary:` comments**, no double casts, no `any`, no non-null assertions, no `@ts-expect-error` in source, one
...
None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, or Redis integration packages is re-exported by `zmdb`.
...
| `@zmdb/repository`         | **MikroORM** EM/repos                  | Auto-validating typed CRUD, `defineRepository`, transactions, populate, read replicas, lifecycle events, entity modeling, and the temporary SQL Server adapter              | aot-validator, schema-core, query-compiler                                                         |
...
| `zmdb`                     | the data/app/web stack                 | Umbrella meta-package (curated root + subpath re-exports; no AI, MCP, OTel, UI, or transport public re-export)                                                              | app, aot-validator, schema-core, query-compiler, repository, sqlite, web; postgres (optional peer) |
...
Counted off a real parse tree, per package, over the **275 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
