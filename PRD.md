...
Thirty published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
lifecycle integrations for React, React Native, Angular, Vue, Svelte, Solid, Next.js, and Nuxt are independently selected and do not place their framework peers on the cohesive product.
...
`@zmdb/vue`, `@zmdb/svelte`, and `@zmdb/solid` are opt-in leaves over `@zmdb/client`, while `@zmdb/next` depends inward on the client and React adapter and `@zmdb/nuxt` depends inward on the client
and Vue adapter. `@zmdb/react-native` also depends inward on the client and React adapter. Each alone declares its framework peers. None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS,
RabbitMQ, or Redis integration packages is re-exported by `zmdb`.
...
Counted off a real parse tree, per package, over the **267 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
