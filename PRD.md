...
Twenty-four published packages. The data, application, and HTTP stack remains one product facade (`zmdb`) combined with the application's selected database vertical; `@zmdb/sqlite` is the complete
...
`@zmdb/client` and `@zmdb/protobuf` are independent dependency-free roots outside this spine. `@zmdb/react` and `@zmdb/vue` are opt-in leaves over `@zmdb/client`; each alone declares its framework
peer. None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, or Redis integration packages is re-exported by `zmdb`.
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Twenty-four published packages in one acyclic DAG, with the schema as the shared source of truth for SQL, database verticals, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, and Redis boundaries (§7 — the seam). |
