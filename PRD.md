...
Thirteen published packages. The data, application, and HTTP stack remains one install (`zmdb`); provider-neutral AI tooling is exposed from `@zmdb/ai` without a provider or framework SDK dependency,
while Anthropic, LangChain, and Vercel integrations ship as separate opt-in packages.
...
`@zmdb/client` and `@zmdb/protobuf` are independent dependency-free roots outside this spine. None of the three opt-in AI integrations is re-exported by `zmdb`.
...
| `@zmdb/schema-core`    | MikroORM entities, Zod/TypeBox schemas | Schema DSL, compile-time type derivation (Entity / Create / Update / read DTOs), relations, custom types, seeding, and OpenAPI. Old `./llm*` paths remain temporary compatibility implementation during extraction. | query-compiler                                                   |
...
| `zmdb`                 | the data/app/web stack                 | Umbrella meta-package (curated root + subpath re-exports; no AI public re-export)                                                                                                                                   | app, aot-validator, schema-core, query-compiler, repository, web |
...
| **RISK-5**  | **Breadth vs. maintainability** — thirteen packages spanning the cohesive product, zero-dependency runtimes, provider-neutral AI tooling, and three opt-in integrations, maintained by a small team.                                                                                                                                                                                                                                                  | Splitting doctrine (`ARCHITECTURE.md` §3.1): subpath exports are the default, a new package is the exception, and dissolving a package is an encouraged refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Thirteen published packages in one acyclic DAG, with the schema as the shared source of truth for the SQL, HTTP, and provider-neutral AI boundaries and opt-in Anthropic, LangChain, and Vercel integrations at the edge (§7 — the seam). |
