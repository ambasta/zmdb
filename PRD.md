...
**Where this stands (2026-09-03).** Twelve of the thirteen requirements are met and
...
build. The one that is not is marked ⚠️ and says what is left: REQ-TF-9's count — five
hand-written descriptors survive in the specs that test the descriptor path, and
`TypeDescriptor` remains a legal _input_. That is not a documentation problem; it has a
gate holding the line at today's number, and retiring the type is plan Phase 7c.
...
generated from. `zmdb`'s root publishes the same `Entity`/`CreateDTO`/`UpdateDTO` as
`zmdb/derive` — one definition, re-exported, taking `Orders` and never the value; there is
no second family keyed by `typeof ordersSchema` (`packages/schema-core/SPEC.md` §4).
