...
> `benchmarks/RESULTS.md` (what is measured), `COOKBOOK.md` (how it is used). **Last revised:** 2026-09-06.
...
an MCP SDK dependency, and `@zmdb/app/otel` adapts caller-owned OpenTelemetry API objects without placing that peer on the cohesive product. Typed gRPC, core NATS, RabbitMQ, and Redis Pub/Sub each
ship as an independently selected transport package with their sole required client peer. Generated-client lifecycle integrations for React, React Native, Angular, Vue, Svelte, Solid, Next.js, Nuxt,
and SvelteKit are independently selected and do not place their framework peers on the cohesive product.
...
| ------------ | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **NestJS**   | `@zmdb/app` + `@zmdb/web`                                     | Controllers, decorators, modules, DI, guards/pipes/interceptors/filters, lifecycle, WS/SSE, OpenAPI, testing utils               | `reflect-metadata`, parameter decorators, `emitDecoratorMetadata`, runtime type reflection, per-request metadata reads |
| **Typia**    | `@zmdb/compiler` + `@zmdb/aot-validator`                      | `is`/`assert`/`validate`/`equals`/`random`, tags, unions, transforms, JSON Ser/De, AOT inlining                                  | dependence on a single bundler path; a separate schema language from the ORM's                                         |
...
>
> The 2026-09-06 count covers 278 shipped files: **53 framework assertions with 54 `// boundary:` comments**, no double casts, no `any`, no non-null assertions, no `@ts-expect-error` in source, one
...
> remaining 53 are trust boundaries, not a claim of zero internal assertions.
>
...
optional and **structurally typed**, so the dependency is never forced. The tooling exception is `oxfmt`, pinned by `@zmdb/migrations` because generated declarations must be formatted by the same
engine as the repository; query compilation and the root, runner, and embedded migration entries do not reach it. One module format, one `exports` map, no `.cjs`.
...
```
@zmdb/query-compiler  (lower-level SQL + database protocols; no external dependency)
        ├────────▶ @zmdb/migrations  (schema lifecycle; oxfmt only for declaration emission)
        ▼
@zmdb/schema-core     (the schema SoT; reuses compiler query/quoting/naming utilities)
        ▼
@zmdb/ai               (provider-neutral tools; depends directly on schema-core)
        ├────────▶ @zmdb/ai/anthropic  (optional Anthropic adapter; depends on ai)
        ├────────▶ @zmdb/ai/langchain  (optional LangChain adapter; depends on ai)
        ├────────▶ @zmdb/ai/vercel     (optional Vercel AI SDK adapter; depends on ai)
        └────────▶ @zmdb/mcp           (transport-neutral MCP; depends only on ai)
        ▼
@zmdb/aot-validator    (compiler-free runtime; direct schema-core dependency)
        ...
        @zmdb/app          (+ direct aot/schema-core/query-compiler deps; one protocol-neutral app graph)
               ...
               │              ├────────▶ @zmdb/jobs-sqlite    (selected SQLite jobs provider)
               │              └────────▶ @zmdb/jobs-postgres  (selected PostgreSQL jobs provider)
               ├────────▶ @zmdb/app/otel               (selected OpenTelemetry adapter; depends only on app)
...
`@zmdb/client`, `@zmdb/client/angular`, and `@zmdb/protobuf` are independent roots outside this spine; Angular accepts the generated client structurally and therefore has no workspace edge.
`@zmdb/client/react`, `@zmdb/client/vue`, `@zmdb/client/svelte`, and `@zmdb/client/solid` are opt-in leaves over `@zmdb/client`, while `@zmdb/client/react-native` depends inward on the client and
React adapter, `@zmdb/next` depends inward on the client and React adapter, `@zmdb/nuxt` depends inward on the client and Vue adapter, and `@zmdb/sveltekit` depends inward on the client and Svelte
adapter. Each alone declares its framework peers. None of the selected AI, UI, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, or Redis integration packages is re-exported by `zmdb`. `@zmdb/mssql` is a
selected database vertical over query-compiler and repository and alone declares the optional SQL Server client peer. Jobs and both jobs providers are likewise selected direct packages rather than
`zmdb` dependencies or facade entries.

### 5.3 Package responsibilities

| Package                     | Replaces                               | Responsibility                                                                                                                                                             | Runtime deps                                                                                                                    |
| --------------------------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `@zmdb/client`              | generated-client transport runtime     | Dependency-free structural HTTP transport, deterministic request planning, response reading, cancellation, authentication injection, and typed errors                      | none                                                                                                                            |
| `@zmdb/client/angular`      | application-local Angular client glue  | Dependency injection, signals, `DestroyRef` cleanup, Observable cancellation, and request-local generated-client ownership                                                 | `@angular/core`, `rxjs` (required peers)                                                                                        |
| `@zmdb/client/react`        | application-local React client glue    | Context, query and mutation hooks with dependency-change cancellation, stale-result suppression, and component-tree isolation                                              | client; `react` (required peer)                                                                                                 |
| `@zmdb/client/react-native` | application-local native client glue   | React hook reuse plus AppState cancellation, explicit offline dispatch policy, and injected connectivity and credential-store ownership                                    | client, react; `react`, `react-native` (required peers)                                                                         |
| `@zmdb/next`                | application-local Next.js client glue  | Request-scoped App Router server clients, selected credential forwarding, request-local memoization, and React browser bindings                                            | client, react, `server-only`; `next`, `react`, `react-dom` (required peers)                                                     |
| `@zmdb/nuxt`                | application-local Nuxt client glue     | Request-scoped Nitro transport, selected credential forwarding, native hydration, and Vue composable reuse                                                                 | client, vue; `nuxt`, `vue` (required peers)                                                                                     |
| `@zmdb/client/vue`          | application-local Vue client glue      | Application plugin plus typed query and mutation composables with watcher/effect-scope cancellation and per-application SSR isolation                                      | client; `vue` (required peer)                                                                                                   |
| `@zmdb/client/svelte`       | application-local Svelte client glue   | Typed context, lazy query and mutation stores, final-subscriber cancellation, stale-result suppression, and request-local server ownership                                 | client; `svelte` (required peer)                                                                                                |
...
| `@zmdb/client/solid`        | application-local Solid client glue    | Context, native resources, owner cancellation, stale-result suppression, and native Suspense/error propagation                                                             | client; `solid-js` (required peer)                                                                                              |
...
| `@zmdb/ai/langchain`        | LangChain tool integration             | Optional `DynamicStructuredTool` fields over the shared AI tool document and validation runtime; sole owner of the `@langchain/core` peer                                  | ai; `@langchain/core` (optional peer)                                                                                           |
| `@zmdb/ai/vercel`           | Vercel AI SDK tool integration         | Structural adapter that injects the installed SDK's branded `jsonSchema`, validates model arguments, and executes the caller's handler                                     | ai; `ai` (optional peer)                                                                                                        |
| `@zmdb/mcp`                 | Model Context Protocol core            | Pure MCP client/server protocol handling, authenticated identity injection, tool validation, and bounded remote calls                                                      | ai                                                                                                                              |
| `@zmdb/protobuf`            | protobuf/gRPC wire runtime             | Dependency-free protobuf calls, descriptors, generated-code wire ABI, and typed gRPC artifacts                                                                             | none                                                                                                                            |
| `@zmdb/aot-validator`       | **Typia runtime**                      | Compiler-free `is`/`assert`/`validate`/`equals`/`random`, tags, unions, transforms, JSON Ser/De                                                                            | schema-core                                                                                                                     |
| `@zmdb/compiler`            | Typia build tooling                    | TypeScript reflection, TypeIR production, AOT emission, project compilation, unplugin/Metro adapters, lint rules, testing utilities, and canonical project config          | ai, aot-validator, query-compiler, schema-core; TypeScript peer                                                                 |
...
| `@zmdb/app`                 | NestJS application kernel              | Stage-3 metadata, dependency injection, modules, lifecycle/extensions, commands, events, CQRS, state, health contracts, and observability ports                            | aot-validator, schema-core, query-compiler, repository                                                                          |
| `@zmdb/jobs`                | NestJS queues and scheduling           | Provider-neutral typed queues, workers, retries, dead letters, scheduling, leases, and `JobStore`/transaction-enqueuer ports                                               | app                                                                                                                             |
| `@zmdb/jobs-sqlite`         | SQLite job storage                     | SQLite queue/lease SQL, migrations, transaction adapter, borrowed durable stores, and the explicit owned-memory convenience store                                          | jobs, sqlite                                                                                                                    |
| `@zmdb/jobs-postgres`       | PostgreSQL job storage                 | PostgreSQL queue/lease SQL, migrations, transaction adapter, and caller-owned pool/client adaptation                                                                       | jobs, postgres; `pg` (required peer)                                                                                            |
| `@zmdb/app/otel`            | OpenTelemetry API integration          | Adapts caller-owned tracers and meters to the app observability ports without owning a provider, SDK, exporter, collector, or ambient context                              | app; `@opentelemetry/api` (required peer)                                                                                       |
| `@zmdb/transport/grpc`      | gRPC server and client integration     | Binds generated protobuf artifacts to typed grpc-js calls, deadlines, metadata, cancellation, streaming and bounded application shutdown                                   | app, protobuf; `@grpc/grpc-js` (required peer)                                                                                  |
...
| `@zmdb/web`                 | NestJS HTTP layer                      | Stage-3 controllers, routing, typed `Ctx`, request pipeline + adapters, guards/pipes/interceptors/filters, DTO validation/serialization, OpenAPI, WS/SSE, and HTTP testing | app, schema-core; compiler and TypeScript (optional tooling peers)                                                              |
| `zmdb`                      | the data/app/web stack                 | Umbrella meta-package (curated root + subpath re-exports; no jobs, jobs provider, AI, MCP, OTel, UI, or transport public re-export)                                        | app, aot-validator, compiler, migrations, schema-core, query-compiler, repository, sqlite, web; mssql/postgres (optional peers) |

### 5.4 Target server product boundary (#645, #753)

The NestJS replacement remains one product, but its default and selected dependency firebreaks are explicit:

| Package               | Product responsibility                                                                                                                         |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `@zmdb/app`           | metadata, DI, modules, lifecycle/extensions, commands, events, CQRS, state, observability ports, health checks and transport-neutral messaging |
| `@zmdb/web`           | HTTP routing/context/pipeline, middleware, body/wire boundaries, OpenAPI, WS/SSE, HTTP testing and HTTP-aware devtools                         |
| `@zmdb/jobs`          | provider-neutral queues, workers, retries, dead letters, scheduling, leases and storage/transaction ports                                      |
| `@zmdb/jobs-sqlite`   | selected SQLite queue/lease persistence, migrations and memory convenience store                                                               |
| `@zmdb/jobs-postgres` | selected PostgreSQL queue/lease persistence, migrations and required `pg` peer                                                                 |

App and web are reached through the default `zmdb` product facade. Jobs is reached directly through `@zmdb/jobs` plus one provider. API identity, catalog/tooling grouping, generated documentation and
release governance make it one cohesive product capability; a mandatory umbrella dependency is not cohesion. There is no runtime `zmdb/jobs` facade or compatibility forwarder.

The core package graph is acyclic: `web -> app`, `jobs -> app`, and neither app nor either sibling imports back. App/web/portable jobs declare only workspace runtime dependencies and Node built-ins
and have no third-party runtime peer. SQLite jobs, PostgreSQL jobs, gRPC, brokers and OpenTelemetry are separately installed adapters that depend inward on app or jobs. `zmdb` cannot reach jobs or a
jobs provider directly, optionally, dynamically, or through private source.
...
### 6.3 Compiler and AOT validator — the Typia replacement (REQ-AV)
...
| **REQ-AV-3** | The compiled path ships as a **wired build step** for the mainstream toolchains — a bundler plugin where there is a bundler, and `@zmdb/compiler` project compilation where there is not — documented end-to-end. ✅ met — see §9.2. | Two consumer fixtures in CI, one per route (`fixtures/consumer-cli/`, `fixtures/consumer-plugin/`): identical accept/reject, identical emitted checks, both measured.                                        |
...
The compiler and validator behavior tests cover the emitted checks. What the AC was reaching for — no interpreter, no descriptor walk, no allocation to answer a yes/no question — holds, and is the
part REQ-AV-2 states directly.
...
| **REQ-WB-12** | OpenAPI documents derive from the compiled `HttpContractIR` shared with routing and generated clients (`toOpenApi`, `serveOpenApi`) — no hand-authored spec or second schema walk.                                     | Changing one contract field changes the served document without a separate OpenAPI edit.                               |
...
| ID            | Requirement                                                                                                                                                                  | AC                                                                                                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **REQ-SV-1**  | Every current `@zmdb/web` public symbol has exactly one owner: app, HTTP web, jobs, an optional package or private benchmark support.                                        | AST export oracle reports 318/318 names once, with no duplicate, omission or extra.                                                                                   |
| **REQ-SV-2**  | `@zmdb/app` owns one application graph, container, construction ledger and lifecycle state machine.                                                                          | Lifecycle tests prove one identity through direct package, web composition and facade imports.                                                                        |
| **REQ-SV-3**  | Extensions start in declaration order, roll back/stop in reverse, share one application grace budget and preserve startup-error precedence.                                  | Runtime tests cover partial start, cleanup failures, concurrent init/dispose and aggregate ordering.                                                                  |
| **REQ-SV-4**  | `@zmdb/web` contains only HTTP-facing concerns and composes `createApp` over `createApplication` without a second lifecycle or request-time extension walk.                  | Import-graph check plus request-path structural test and existing route benchmark guard.                                                                              |
| **REQ-SV-5**  | `@zmdb/jobs` owns provider-neutral queues/scheduling and domain-shaped storage ports; SQLite lives only in `@zmdb/jobs-sqlite` and PostgreSQL only in `@zmdb/jobs-postgres`. | Packed jobs-only and provider consumers prove behavior, migrations, transactions, leases, and borrowed-resource survival.                                             |
| **REQ-SV-6**  | App, web and jobs declare no third-party runtime peer, no forbidden reverse edge and no private-source cross-package import.                                                 | Mechanical package-DAG verifier.                                                                                                                                      |
| **REQ-SV-7**  | Moved paths are removed with no forwarding files or deprecated aliases.                                                                                                      | Old-specifier resolution tests fail and a source scan finds no forwarder.                                                                                             |
| **REQ-SV-8**  | `zmdb`, `zmdb/app`, and `zmdb/web` are explicit default re-exports; jobs has no `zmdb/jobs` facade and providers compose the direct `@zmdb/jobs` identities.                 | Packed consumers assert imports/typechecks, absence of `zmdb/jobs*`, and `===` identity outside the workspace.                                                        |
| **REQ-SV-9**  | The package move preserves hot paths.                                                                                                                                        | Same-machine before/after raw samples retain the existing framework contract; an unexplained median startup or request-throughput regression above 5% blocks closure. |
| **REQ-SV-10** | Installing packed `zmdb` excludes jobs and its providers; installing portable jobs excludes every concrete store; each provider is selected explicitly.                      | Installed-manifest graph verifier reports exact metadata-derived closures, peers, forbidden edges, and clean-consumer failure paths.                                  |
...
**Where this stands (2026-09-03).** All thirteen requirements are met, and each row below names the script or test that enforces it.
...
| **REQ-TF-6**  | `Sensitive` is enforced by the **type**: `ReadDTO<T>` cannot name a sensitive column, so a leak is a compile error rather than a serializer responsibility.                                                                                                                                                                                      | ✅ Met, by both routes. `derive/tagged-dto.type-test.ts` pins `SensitiveKeys<User>` and `@ts-expect-error`s the read; `packages/compiler/src/reflect/documents.spec.ts` ("never names a sensitive column, in any document or anywhere in the output") greps the whole emitted module, not just the schemas, because a column dropped from a document and left in a hoisted constant is still a leak. `Entity<T>` and `CreateDTO<T>` deliberately **keep** the column — the repository has to write and redact it — so `jsonSchemaFromShape` filters at the last step in every variant.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **REQ-TF-7**  | The JSON Schema suite is type-driven with unchanged output contracts: `toJsonSchema<T>()` and the `entity`/`create`/`update`/`get`/`list`/`search` variants, `toJsonSchemaWithRelations`, `toOpenApiComponents`, `toListSchema`, `toSearchSchema`, `toolFromSchema`.                                                                             | ✅ Met. `packages/compiler/src/reflect/documents.spec.ts` ("`toJsonSchema<T>` vs `toJsonSchema(schema, variant)`") asserts byte-identical documents for both routes, that the fixture's every document is covered, that no call survives the transform, and that each distinct document is hoisted once and frozen all the way down. `toJsonSchema<T>()` produces the `entity` variant; a request body is the two-argument form, because a variant name is a value. `toJsonSchemaWithRelations` produces the same document from one fewer argument: the relations come off `schema.ir`, so a generated document cannot name a relation its table does not declare.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **REQ-TF-8**  | The AOT transformer resolves types with the **TypeScript checker**, not a text parser, and generates the runtime checks the tags declare — bounds, lengths, patterns, integrality, enum membership, nullability.                                                                                                                                 | ✅ Met. `parseType` is deleted — `grep -r parseType packages/` returns nothing — and `packages/compiler/src/reflect/reflect.spec.ts` plus `packages/compiler/src/emit/__testing__/project.ts` drive the real checker over a temp project for `is`/`assert`/`validate`/`equals`/`random`/`toJsonSchema`/`schemaOf` on named types, mapped types and derived DTOs. `packages/compiler/src/reflect/schema-values.spec.ts` ("a declaration a table cannot be made of") pins the refusals, which are by name rather than by silence. One deviation from DESIGN §8 item 4: `transformCode` survives, because rule-first `validate(tags.X, expr)` is a value-shaped call the checker path has no reason to handle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **REQ-TF-9**  | No `TypeDescriptor` is hand-written anywhere — not in the packages, not in the benchmark harness.                                                                                                                                                                                                                                                | ✅ Met, and at zero rather than ratcheted. `yarn verify:no-descriptors` scans 871 files with an empty allow-list and finds none. The type itself is deleted: there is no `TypeDescriptor`, no `RuntimeSchema` union and no `irFromDescriptor` bridge, so `is`/`assert`/`validate`/`equals`/`assertEquals`/`random`/`issuesFor`/`assertStringify`/`decode` all take a `TypeIR` and nothing else — a shape that is generated, from a type the compiler checked, rather than a mirror of one written out by hand. The four specs that tested the legacy front-end build the IR instead, and the gate's third signal is a _declaration_ of the name, so the shape cannot come back by being re-declared somewhere new. The `PARTIAL_ON_PURPOSE` exemption in `verify:tf-coverage` is empty for the same reason: the only shape that could not express `maximum` no longer exists.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **REQ-TF-10** | The runtime schema value the query compiler needs for SQL is **generated** from the type, not authored. Types may generate runtime data; they are not the runtime data.                                                                                                                                                                          | ✅ Met. `packages/compiler/src/reflect/schema-values.spec.ts` pins what `schemaOf<T>()` emits; `packages/schema/src/schema-of.type-test.ts` asserts the generated value derives the same types as the declaration it came from; `packages/orm/src/tagged-schema.type-test.ts` does it from the caller's side (`defineRepository(schemaOf<User>(), driver)`); and `packages/orm/src/tagged-to-ddl.spec.ts` takes it through to DDL per dialect — including the two places the snapshot format loses information, which it asserts rather than hides.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **REQ-TF-11** | Checker cost is paid **once per build**, not once per file.                                                                                                                                                                                                                                                                                      | ✅ Met. `packages/compiler/src/unplugin/plugin.spec.ts` asserts one `API` instance for the bundler path; `yarn verify:build-budget` does it for project compilation over a generated 64-module project, and adds the sharper form of "not once per file" — the snapshot-update log is identical at 8 modules and at 64. Build time is published in that script's output. See the note below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **REQ-TF-12** | There is exactly **one** way to declare a table. `defineSchema` is removed rather than maintained alongside the tagged form, and a codemod converts an existing project. Until it is removed it is a proving device: the IR derived from the value must equal the IR derived from the type.                                                      | ✅ Met, and the proving device has served its purpose and gone. `yarn verify:no-defineschema` imports all seven published surfaces and reads their **export names** — exact rather than syntactic — then scans every file `git ls-files` knows about, committed or not, for `defineSchema`/`irFromSchema` used as code; `ALLOWED` is the codemod, its four corpora, four one-shot issue scripts and one docs page title, and a stale entry fails too. `packages/compiler/src/reflect/codemod.spec.ts` converts the corpus and asserts the output line for line. The equivalence test is gone with the value side.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **REQ-TF-13** | A column has **three** types and each layer renders the one it owns: wire (what arrives over HTTP), app (what handler code sees), db (what the dialect declares). `timestamp` is an ISO-8601 `string` in JSON Schema, a `Date` in `Entity<T>`, and `timestamptz` in Postgres DDL. The IR stores the abstract type; dialects render the spelling. | ✅ Met. `packages/zmdb/src/three-types.spec.ts` is the cross-package test — one case per dialect, asserting the DDL spelling, the `Entity<T>` validator and the JSON Schema for one `timestamp` column agree; `packages/migrations/src/sql-types.spec.ts` pins `ddlType` per dialect (`TIMESTAMPTZ` on Postgres, `DATETIME(3)` on MySQL, `TEXT` on SQLite) and `packages/compiler/src/reflect/payload-types.spec.ts` pins `objectTypeFromIR` against the derived type. No code path accepts `Date \| string`. The wire layer _checks_ the ISO string rather than only declaring it: a `format` is an annotation in JSON Schema and neither validator walk reads one, so it is lowered to a `pattern` — RFC 3339, offset required, because an offset-less string is read as local time and that is the lost-offset bug `timestamptz` prevents. `int64` reuses the decoder's expression, so the wire validator accepts exactly what `decodeWireValue` can convert. Three renderings from one walk is the whole claim, so `yarn verify:one-walker` gates it: 1,129 source files are scanned; outside `packages/schema/src/ir/`, four shipped sources name the metadata vocabulary, 20 read normalized column meaning, and one is the web compiler envelope reader, each with the reason written next to it, and a stale exemption fails too.                                                                                                                                                     |
...
It was removed in the same way as the other four: generation now starts from the IR and uses the emitter's sampler. This also moved seeding to `@zmdb/orm/seeding`, because the sampler lives in
`@zmdb/validator` and `@zmdb/schema` cannot depend on it without reversing the package graph. No existing check found the extra walker.
...
All four are covered now: `Fts<'users_fts'>` went into `packages/compiler/src/reflect/__fixtures__/tables.ts`, where the golden pins the `ftsTable` it produces, and the boolean spelling `Fts<true>`
plus the two missing cardinalities went in beside it as assertions of their own. `Fts<true>` is also declared in `ir.spec.ts`, `tagged-to-ddl.spec.ts` and the codemod's corpus, so the two spellings
are checked at every stage they pass through.
...
`packages/schema/src/derive/type-derivation-tagged.type-test.ts`, which is `type-derivation.type-test.ts` with `S` rebound.
...
different types. `packages/schema/src/tags/serial-foreign-key.type-test.ts` pins both halves.
...
That is what "not once per file" means operationally, because telling the compiler about a new file re-checks it, and it is why `packages/compiler/src/codegen/index.ts` writes every witness before
transforming any of them — an ordering that reads like an arbitrary choice and is the only reason the second number is 2 instead of 65. Moving those two calls inside the loop is a one-line tidy-up
that takes the log to 65 updates; the gate fails.
...
| **REQ-AV-3** | ✅ met (2026-09-02)           | Closed by the second route. A bundler plugin only helps a project that has a bundler, so `@zmdb/compiler` project compilation writes the checks beside the source and rewrites the call sites to import them — no plugin, no transform, no bundler, runnable under plain `node`. Both routes are consumer fixtures in CI (`fixtures/`), and they emit the **same code**: `is<Order>` measured **27.1M ops/s** through the no-bundler route and **33.3M** through the plugin, a spread attributed to measurement order rather than route. |
...
| **REQ-NF-12** | Publishing: ESM-only, single `exports` map, Trusted Publishing (OIDC) with provenance, `latest` tracking highest-precedence release. License **MPL-2.0**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Counted off a real parse tree, per package, over the **278 shipped source files** in `packages/*/src`. Tests are excluded and the script says why: `*.spec.ts`, `*.type-test.ts`, `__testing__/` and
...
| Metric                                     | 2026-08-31 (grep, 67 files) | 2026-09-02 (parse tree, 84 files) | 2026-09-03 (DSL deleted) | 2026-09-06 current | Ceiling |
| ------------------------------------------ | --------------------------: | --------------------------------: | -----------------------: | -----------------: | ------: |
| `: any` / `<any>` / `any[]` / `as any`     |                       **0** |                             **0** |                    **0** |              **0** |       0 |
| `@ts-expect-error` / `@ts-ignore` in src   |                       **0** |                             **0** |                    **0** |              **0** |       0 |
| `as unknown as` double casts               |                       **1** |                             **2** |                    **1** |              **0** |       0 |
| Type assertions (`as T`, excl. `as const`) |                      **28** |                            **65** |                   **62** |             **53** |      53 |
| `// boundary:` comments                    |                      **37** |                            **56** |                   **54** |             **54** |       — |
...
to 55. Removing the obsolete OpenAPI metadata traversal then removed two more boundary casts, taking the current ceiling to 53.
...
- **Assertions 28 → 65.** The tree grew by 18 files: the reflection front-end, the emitter, the codegen CLI, the `zmdb-codemod` and the test-time bridge — the whole type-first spine, which did not
  ...
  The 65th is the one raise in this table, and it is a raise rather than a swap, so it needs its own sentence: `schemasFrom<{ User: User }>(url, ['User'])` in `packages/compiler/src/testing` returns
...
Per-package distribution of the 2026-09-06 recount:

| Package                     | Assertions | `// boundary:` | What the boundaries are                                                              |
| --------------------------- | ---------: | -------------: | ------------------------------------------------------------------------------------ |
| `@zmdb/client`              |          0 |              0 | —                                                                                    |
| `@zmdb/client/angular`      |          0 |              0 | —                                                                                    |
| `@zmdb/client/react`        |          0 |              0 | —                                                                                    |
| `@zmdb/client/react-native` |          0 |              0 | —                                                                                    |
| `@zmdb/next`                |          0 |              0 | —                                                                                    |
| `@zmdb/nuxt`                |          0 |              0 | —                                                                                    |
| `@zmdb/client/vue`          |          0 |              0 | —                                                                                    |
| `@zmdb/client/svelte`       |          0 |              0 | —                                                                                    |
| `@zmdb/client/solid`        |          0 |              0 | —                                                                                    |
| `@zmdb/schema-core`         |         11 |              9 | untrusted DTO payload reads, the custom-type registry                                |
| `@zmdb/ai`                  |          1 |              1 | parsed OpenAPI request payloads                                                      |
| `@zmdb/ai/anthropic`        |          0 |              0 | —                                                                                    |
| `@zmdb/ai/langchain`        |          0 |              0 | —                                                                                    |
| `@zmdb/ai/vercel`           |          0 |              0 | —                                                                                    |
| `@zmdb/mcp`                 |          0 |              0 | —                                                                                    |
| `@zmdb/aot-validator`       |         26 |             16 | checker `Type` → `TypeReference`, `JSON.parse` → `T`, one certified validator return |
| `@zmdb/protobuf`            |          0 |              0 | —                                                                                    |
| `@zmdb/repository`          |          5 |              6 | driver row → `Entity<S>`, subclass statics, and cursor payloads                      |
| `@zmdb/query-compiler`      |          1 |              1 | the `compile()` duck-type guard                                                      |
| `@zmdb/mssql`               |          0 |              0 | —                                                                                    |
| `@zmdb/postgres`            |          0 |              0 | —                                                                                    |
| `@zmdb/sqlite`              |          1 |              1 | validated structural driver result                                                   |
| `@zmdb/app`                 |          3 |             10 | metadata slots, DI token → instance, and state branding                              |
| `@zmdb/jobs`                |          0 |              1 | runtime queue payload guard                                                          |
| `@zmdb/jobs-postgres`       |          0 |              0 | —                                                                                    |
| `@zmdb/app/otel`            |          0 |              0 | —                                                                                    |
| `@zmdb/transport/grpc`      |          0 |              0 | —                                                                                    |
| `@zmdb/transport/nats`      |          0 |              0 | —                                                                                    |
...
| ID          | Risk                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RISK-1**  | ~~**The AOT premise is unearned out of the box.**~~ **Closed 2026-09-02.** The premise needed a route that did not assume a bundler, and `@zmdb/compiler` project compilation is it: it compiles checks to files next to the source, rewrites call sites to import them, and check mode fails a build whose committed output is stale. Both routes are fixtures in CI, emitting the same code (§9.2).                                                 | Compiler integration tests exercise the generated program and Metro bundle. **Residual risk:** a consumer who wires neither route gets a loud missing-witness failure at runtime, so documentation and the build-path smoke test remain part of the contract.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **RISK-5**  | **Breadth vs. maintainability** — thirty-eight packages spanning the cohesive product, compiler tooling, complete database verticals, zero-dependency runtimes, provider-neutral AI tooling, selected provider integrations, MCP, UI adapters, gRPC, NATS, RabbitMQ, Redis, and OpenTelemetry adaptation, maintained by a small team.                                                                                                                 | Splitting doctrine (`ARCHITECTURE.md` §3.1): subpath exports are the default, a new package is the exception, and dissolving a package is an encouraged refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **RISK-8**  | **`defineSchema` inverted P2** — resolved 2026-09-03. The column facts lived in a runtime value and the types were derived from it, so the runtime data was the source of truth and the type system was downstream: the opposite of what P2 and P3 claim. It was also why the AOT transformer could not see a named type, and why `benchmarks/harness/framework/app.ts` hand-wrote a `TypeDescriptor`.                                                | Type-first declaration (§6.7, `REQ-TF-*`, `DESIGN-type-first.md`) shipped: the tags are on the interface, the TS 7 checker resolves it, and both the checks and the runtime schema value are generated from that. `defineSchema` was deleted rather than kept as a peer — `yarn verify:no-defineschema` fails if it comes back — and `packages/compiler/src/reflect/codemod.spec.ts` carries an existing project across. Nothing named here is still open: the hand-written descriptor is gone from the harness and from the four spec files that used to test that path, and `TypeDescriptor` itself is deleted — `verify-tf-coverage.mjs` tolerates no partial reader, because the shape that needed the tolerance no longer exists. |
| **OPEN-1**  | ~~Should the DI container be promoted from a `@zmdb/web` sub-module to its own package?~~ **Resolved by #645.**                                                                                                                                                                                                                                                                                                                                       | DI moves with modules and lifecycle into `@zmdb/app`; it does not become a standalone package because the three form one application kernel and would otherwise require bidirectional edges.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Both PRDs: four packages / a data layer with a separate framework                                                                                                                                | **§5.** Thirty-eight published packages in one acyclic DAG, with the schema as the shared source of truth for SQL, database verticals, HTTP, provider-neutral AI, opt-in Anthropic/LangChain/Vercel integrations, MCP, OpenTelemetry, gRPC, NATS, RabbitMQ, Redis, compiler tooling, and UI-adapter boundaries (§7 — the seam). |
