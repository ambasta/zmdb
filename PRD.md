...
**REQ-AV-1's AC, amended 2026-09-02.** It read _"contains no function call and no object literal
on the happy path"_, and taken literally that is not achievable — nor desirable. Two calls
survive on purpose, and the AC now names both rather than being quietly treated as met:

- **JS intrinsics.** A check for `number` is `typeof x === 'number' && !Number.isNaN(x)`; a
  check that a value is an object and not an array calls `Array.isArray`. Those calls _are_ the
  check. A hand-written predicate would make exactly the same two, and inlining them further
  would mean emitting slower code to satisfy a sentence.
- **One hoisted helper per named type.** The emitter inlines an anonymous type and hoists a named
  one, because a name is the signal that a type may recur or appear twice — a self-referential
  interface has no inline form at all, and duplicating a shared one multiplies bundle size by its
  use count. So `is<{ n: number }>(x)` is one boolean expression with no zmdb call in it, and
  `is<User>(x)` is one call to one `_zmdbCheckUser` shared by every site.

Both halves are asserted, on the real bundle, in
`packages/aot-validator/src/cli/consumer-fixtures.spec.ts`. What the AC was reaching for — no
interpreter, no descriptor walk, no allocation to answer a yes/no question — holds, and is the
...
**Where this stands (2026-09-03).** Eleven of the thirteen requirements are met and
each row below names the script or test that enforces it, rather than asserting the
outcome. `yarn verify:tf-acceptance` audits that claim: every row has to name a gate or
a spec, every file and every quoted test name it cites has to exist, and every gate it
cites has to be a script `ci.yml` actually runs. A row nobody can execute fails the
build. The two that are not are marked ⚠️ and say what is left: REQ-TF-4's second
sentence (the root DTO types still branch on a schema value, because `./dto` is
parameterised that way) and REQ-TF-9's count (five hand-written descriptors survive in
the specs that test the descriptor path, and `TypeDescriptor` remains a legal _input_).
Neither is a documentation problem — each has a gate holding the line at today's number,
and each is a coding task with a named owner in `packages/schema-core/SPEC.md` §4 and
plan Phase 7c respectively.
...
| **REQ-TF-4**  | The whole DTO suite derives from the tagged type: `Entity`, `CreateDTO`, `UpdateDTO`, `WhereDTO`, `ReadDTO`, `PrimaryKeyOf`, order-by, pagination, projection, and populated/join shapes. Each takes the tagged type only — no dispatch on a schema value.                                                                                       | ⚠️ Met for the derivations, not yet for the dispatch. `derive/type-derivation-tagged.type-test.ts` is the REQ-SC-2…5 assertions with `S` rebound to the tagged interface — identical key sets and optionality, and every derivation mutually assignable with the value-side derivation, which survives inside that file as two column-map types now that `defineSchema` is gone — and `derive/tagged-dto.type-test.ts` covers the suite directly. Not literally _unchanged_; that contradicted REQ-TF-5, see the note below. **The second sentence is outstanding:** the root `Entity`/`CreateDTO`/`UpdateDTO` still branch on `S extends TaggedSchema<infer T>` because `./dto` is parameterised on the schema value. Collapsing that branch is `schema-core/SPEC.md` §4. |
...
| **REQ-TF-9**  | No `TypeDescriptor` is hand-written anywhere — not in the packages, not in the benchmark harness.                                                                                                                                                                                                                                                | ⚠️ Ratcheted, not zero. `yarn verify:no-descriptors` scans 305 files and finds 5 hand-written descriptors across 4 files, all of them the specs that test the descriptor path itself — the ceiling is the count, so a sixth fails. `TypeDescriptor` survives as a legacy _input_ (`RuntimeSchema = TypeIR \| TypeDescriptor`), which is why the number is not yet zero; retiring the type is Phase 7c, and the `PARTIAL_ON_PURPOSE` allowance in `verify:tf-coverage` goes with it.                                                                                                                                                                                                                                                                                        |
...
REQ-TF-3 claims a tag costs the compiler nothing, and the acceptance criterion asks for
typecheck wall-time against an untagged baseline. `yarn verify:instantiations` generates both
sides from one code path — 512 tables, 4,096 tagged columns, and the same 512 interfaces with
...
The criterion asks for one `API` instance across a multi-file build plus a published build-time
measurement. `yarn verify:build-budget` writes a 64-module project — each module a tagged
...
Five of those six are types, from `zmdb/derive`, applied to the declaration itself; they
contribute nothing to a bundle. The sixth is the only value in the list — an object literal
the transformer writes at build time, which is what the DDL and the inlined validators are
generated from. `zmdb`'s root publishes `Entity`/`CreateDTO`/`UpdateDTO` a second time
taking a schema _value_ type, for code parameterised on `typeof ordersSchema` rather than on
`Orders`; the two agree, and `packages/schema-core/SPEC.md` §4 tracks collapsing them.
...
export class OrderRepository extends BaseRepository<typeof ordersSchema> {
...
const OrdersRepo = repositoryToken<typeof ordersSchema>('OrdersRepo');

@Controller('/orders')
class OrderController {
  @Inject(OrdersRepo)
  orders!: BaseRepository<typeof ordersSchema>; // type must match the token — no cast
...
2. `defineRepository(schemaOf<T>(), driver, { dialect })` (one call) — or subclass
   `BaseRepository` for domain queries.
...
**REQ-DX-2:** The developer's total edit surface for a column change is **the file holding
the interface, plus whatever genuinely needed a decision** — never a mechanical re-typing of the same
...
"zero escape hatches": the survivors are heterogeneous `Map`s, `Function.prototype.constructor`,
...
5. Consumer-facing code in every doc example contains **zero `as`** (✅ **0 violations** —
   §9.4).
6. Every framework assertion carries a `// boundary:` comment (✅ **64/64** — §9.4) **and the
   CI counter is wired** (✅ `yarn verify:escape-hatches`, 2026-09-02) and the `new Function`
   guard passes (✅ **0 sites**, checked by the same script, §9.5).
7. `REQ-AV-3` is closed (✅ **2026-09-02**, §9.2): there is a documented build step for a
   project with a bundler and for a project without one, both covered by a consumer fixture in
   CI, and both emitting the same compiled checks.
