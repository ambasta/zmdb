...
| **REQ-TF-4**  | The whole DTO suite derives from the tagged type: `Entity`, `CreateDTO`, `UpdateDTO`, `WhereDTO`, `ReadDTO`, `PrimaryKeyOf`, order-by, pagination, projection, and populated/join shapes. Each takes the tagged type only — no dispatch on a schema value.                                                                                       | The existing REQ-SC-2…REQ-SC-5 type tests pass unchanged with the tagged interface substituted for `typeof Schema`.                                                                                 |
