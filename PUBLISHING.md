...
This section is the normative target for epic #721. The current workflow still repeats twenty-three package names in release scripts and has no root `CHANGELOG.md` or release-plan module; #728
...
1. Bump the version in all twenty-three published `packages/*/package.json` files (and `VERSION` in `prepare-publish.mjs`), commit.
2. Tag and push:
   ```bash
