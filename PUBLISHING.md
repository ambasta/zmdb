...
This section is the normative target for epic #721. The current workflow still repeats nineteen package names in release scripts and has no root `CHANGELOG.md` or release-plan module; #728 implements
...
2. **Configure a Trusted Publisher for each package.** On npmjs.com → your package → **Settings → Trusted Publisher → GitHub Actions**, enter:
   ...
   >   mcp protobuf aot-validator repository sqlite app jobs otel transport-grpc web zmdb
   > )
...
1. Bump the version in all nineteen published `packages/*/package.json` files (and `VERSION` in `prepare-publish.mjs`), commit.
2. Tag and push:
   ```bash
