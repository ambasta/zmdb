...
Issue #728 implements the release tooling that is checked in today: all 38 public packages move as one train under one `v<version>` tag. Issue #746 freezes its replacement in
[`scripts/release/SPEC.md`](./scripts/release/SPEC.md): eight core packages remain cohesive, 28 integrations and two tooling packages become independent release units, and compatibility ranges come
