...
This section is the normative target for epic #721. The current workflow still repeats twenty-five package names in release scripts and has no root `CHANGELOG.md` or release-plan module; #728
...
2. **Configure a Trusted Publisher for each package.** On npmjs.com → your package → **Settings → Trusted Publisher → GitHub Actions**, enter:
   ...
   >
   > packages=(
   >   client react angular vue query-compiler schema-core ai ai-anthropic ai-langchain ai-vercel
...
1. Bump the version in all twenty-five published `packages/*/package.json` files (and `VERSION` in `prepare-publish.mjs`), commit.
2. Tag and push:
   ```bash
