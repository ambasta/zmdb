...
2. **Configure a Trusted Publisher for each package.** On npmjs.com → your package → **Settings → Trusted Publisher → GitHub Actions**, enter:
   ...
   >
   > packages=(
   >   client query-compiler schema-core ai ai-anthropic ai-langchain
...
1. Bump the version in all twelve `packages/*/package.json` files (and `VERSION` in `prepare-publish.mjs`), commit.
2. Tag and push:
   ```bash
